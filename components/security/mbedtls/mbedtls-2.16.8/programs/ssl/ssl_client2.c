...
#if !defined(MBEDTLS_ENTROPY_C) || !defined(MBEDTLS_SSL_TLS_C) || !defined(MBEDTLS_SSL_CLI_C) || !defined(MBEDTLS_NET_C) \
...
#define USAGE_RECSPLIT "    recsplit=0/1        default: (library default: on)\n"
#else
#define USAGE_RECSPLIT
#endif

#if defined(MBEDTLS_DHM_C)
#define USAGE_DHMLEN "    dhmlen=%%d           default: (library default: 1024 bits)\n"
#else
#define USAGE_DHMLEN
#endif
...
#define USAGE_FALLBACK "    fallback=0/1        default: (library default: off)\n"
#else
...
#define USAGE3                                                                                                                     \
    ...
    "    reconnect_hard=%%d   default: 0 (disabled)\n" USAGE_TICKETS USAGE_MAX_FRAG_LEN USAGE_TRUNC_HMAC USAGE_ALPN USAGE_FALLBACK \
        USAGE_EMS USAGE_ETM USAGE_CURVES USAGE_RECSPLIT USAGE_DHMLEN "\n"
#define USAGE4                                                                      \
...
int main(int argc, char* argv[])
{
        ...
        if (strcmp(p, "none") == 0) {
            ...
            while (i < CURVE_LIST_SIZE - 1 && *p != '\0') {
                ...
                if ((curve_cur = mbedtls_ecp_curve_info_from_name(q)) != NULL) {
                    ...
                    for (curve_cur = mbedtls_ecp_curve_list(); curve_cur->grp_id != MBEDTLS_ECP_DP_NONE; curve_cur++) {
...
#if defined(MBEDTLS_CERTS_C)
    {
        ...
        if (ret == 0)
            ...
            for (i = 0; mbedtls_test_cas_der[i] != NULL; i++) {
                ret = mbedtls_x509_crt_parse_der(&cacert, (const unsigned char*)mbedtls_test_cas_der[i], mbedtls_test_cas_der_len[i]);
                if (ret != 0)
                    break;
            }
    }
...
#endif
#if defined(MBEDTLS_CERTS_C)
        ret = mbedtls_pk_parse_key(&pkey, (const unsigned char*)mbedtls_test_cli_key, mbedtls_test_cli_key_len, NULL, 0);
#else
    {
        ret = 1;
        mbedtls_printf("MBEDTLS_CERTS_C not defined.");
    ...
    mbedtls_printf("  . Connecting to %s/%s/%s...", opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM ? "tcp" : "udp", opt.server_addr,
                   opt.server_port);
    fflush(stdout);

    if ((ret = mbedtls_net_connect(&server_fd, opt.server_addr, opt.server_port,
                                   opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM ? MBEDTLS_NET_PROTO_TCP : MBEDTLS_NET_PROTO_UDP))
        != 0) {
    ...
    if (opt.recsplit != DFL_RECSPLIT)
        mbedtls_ssl_conf_cbc_record_splitting(&conf, opt.recsplit ? MBEDTLS_SSL_CBC_RECORD_SPLITTING_ENABLED
