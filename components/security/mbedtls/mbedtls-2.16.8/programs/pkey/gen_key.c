...
#if defined(MBEDTLS_PK_WRITE_C) && defined(MBEDTLS_FS_IO) && defined(MBEDTLS_ENTROPY_C) && defined(MBEDTLS_CTR_DRBG_C)
...
#if !defined(MBEDTLS_PK_WRITE_C) || !defined(MBEDTLS_PEM_WRITE_C) || !defined(MBEDTLS_FS_IO) || !defined(MBEDTLS_ENTROPY_C) \
...
int main(int argc, char* argv[])
{
    ...
    if (opt.use_dev_random) {
        if ((ret = mbedtls_entropy_add_source(&entropy, dev_random_entropy_poll, NULL, DEV_RANDOM_THRESHOLD, MBEDTLS_ENTROPY_SOURCE_STRONG))
            != 0) {
    ...
    if (opt.type == MBEDTLS_PK_RSA) {
        ...
        ret = mbedtls_ecp_gen_key((mbedtls_ecp_group_id)opt.ec_curve, mbedtls_pk_ec(key), mbedtls_ctr_drbg_random, &ctr_drbg);
        if (ret != 0) {
