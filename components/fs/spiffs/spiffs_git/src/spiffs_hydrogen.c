...
s32_t SPIFFS_mount(spiffs* fs, spiffs_config* config, u8_t* work, u8_t* fd_space, u32_t fd_space_size, void* cache, u32_t cache_size,
                   spiffs_check_callback check_cb_f)
{
    SPIFFS_API_DBG("%s "
                   ...
                   " cachesz:"_SPIPRIi
                   "\n",
                   __func__, SPIFFS_CFG_PHYS_SZ(fs), SPIFFS_CFG_LOG_PAGE_SZ(fs), SPIFFS_CFG_LOG_BLOCK_SZ(fs), SPIFFS_CFG_PHYS_ERASE_SZ(fs),
        ...
        if (len < (s32_t)SPIFFS_CFG_LOG_PAGE_SZ(fs)) {
            ...
            if (fd->cache_page) {
                    ...
                    offset + len > fd->cache_page->offset + SPIFFS_CFG_LOG_PAGE_SZ(fs)) // writing beyond cache page
                {
                    ...
                    SPIFFS_CACHE_DBG("CACHE_WR_DUMP: dumping cache page "_SPIPRIi
                                     " for fd "_SPIPRIfd
                                     ":"_SPIPRIid
            ...
            if (alloc_cpage) {
                ...
                if (fd->cache_page) {
                    ...
                    SPIFFS_CACHE_DBG("CACHE_WR_ALLO: allocating cache page "_SPIPRIi
                                     " for fd "_SPIPRIfd
                                     ":"_SPIPRIid
                                     "\n",
            ...
            if (fd->cache_page) {
                ...
                SPIFFS_CACHE_DBG("CACHE_WR_WRITE: storing to cache page "_SPIPRIi
                                 " for fd "_SPIPRIfd
                                 ":"_SPIPRIid
                                 ", offs "_SPIPRIi
                                 ":"_SPIPRIi
...
static s32_t spiffs_stat_pix(spiffs* fs, spiffs_page_ix pix, spiffs_file fh, spiffs_stat* s)
{
    ...
    u32_t obj_id_addr =
        SPIFFS_BLOCK_TO_PADDR(fs, SPIFFS_BLOCK_FOR_PAGE(fs, pix)) + SPIFFS_OBJ_LOOKUP_ENTRY_FOR_PAGE(fs, pix) * sizeof(spiffs_obj_id);
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_READ, fh, obj_id_addr, sizeof(spiffs_obj_id), (u8_t*)&obj_id);
    SPIFFS_API_CHECK_RES(fs, res);
    ...
    if ((fd->flags & SPIFFS_O_DIRECT) == 0) {
        ...
        if (fd->cache_page) {
            SPIFFS_CACHE_DBG("CACHE_WR_DUMP: dumping cache page "_SPIPRIi
                             ...
                             "\n",
                             fd->cache_page->ix, fd->file_nbr, fd->obj_id, fd->cache_page->offset, fd->cache_page->size);
            res = spiffs_hydro_write(fs, fd, spiffs_get_cache_page(fs, spiffs_get_cache(fs), fd->cache_page->ix), fd->cache_page->offset,
                                     fd->cache_page->size);
            if (res < SPIFFS_OK) {
                fs->err_code = res;
            }
...
s32_t SPIFFS_rename(spiffs* fs, const char* old_path, const char* new_path)
{
    ...
    res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, 0, (const u8_t*)new_path, 0, 0, &pix_dummy);
...
s32_t SPIFFS_update_meta(spiffs* fs, const char* name, const void* meta)
{
    ...
    res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, 0, 0, meta, 0, &pix_dummy);
...
static s32_t spiffs_read_dir_v(spiffs* fs, spiffs_obj_id obj_id, spiffs_block_ix bix, int ix_entry, const void* user_const_p,
                               void* user_var_p)
{
    (void)user_const_p;
    s32_t res;
...
struct spiffs_dirent* SPIFFS_readdir(spiffs_DIR* d, struct spiffs_dirent* e)
{
    ...
    res = spiffs_obj_lu_find_entry_visitor(d->fs, d->block, d->entry, SPIFFS_VIS_NO_WRAP, 0, spiffs_read_dir_v, 0, e, &bix, &entry);
    if (res == SPIFFS_OK) {
        d->block = bix;
        d->entry = entry + 1;
...
s32_t SPIFFS_vis(spiffs* fs)
{
    ...
    while (bix < fs->block_count) {
        ...
        res = _spiffs_rd(fs, SPIFFS_OP_C_READ | SPIFFS_OP_T_OBJ_LU2, 0, SPIFFS_ERASE_COUNT_PADDR(fs, bix), sizeof(spiffs_obj_id),
                         (u8_t*)&erase_count);
        SPIFFS_CHECK_RES(res);
