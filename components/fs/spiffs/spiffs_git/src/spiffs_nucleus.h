...
#define SPIFFS_MAGIC(fs, bix) ((spiffs_obj_id)(0x20140529 ^ SPIFFS_CFG_LOG_PAGE_SZ(fs)))
#else  // SPIFFS_USE_MAGIC_LENGTH
#define SPIFFS_MAGIC(fs, bix) ((spiffs_obj_id)(0x20140529 ^ SPIFFS_CFG_LOG_PAGE_SZ(fs) ^ ((fs)->block_count - (bix))))
...
#define SPIFFS_HAL_WRITE(_fs, _paddr, _len, _src) (_fs)->cfg.hal_write_f((_fs), (_paddr), (_len), (_src))
#define SPIFFS_HAL_READ(_fs, _paddr, _len, _dst)  (_fs)->cfg.hal_read_f((_fs), (_paddr), (_len), (_dst))
#define SPIFFS_HAL_ERASE(_fs, _paddr, _len)       (_fs)->cfg.hal_erase_f((_fs), (_paddr), (_len))
...
#define spiffs_get_cache_page_hdr(fs, c, ix) ((spiffs_cache_page*)(&((c)->cpages[(ix) * SPIFFS_CACHE_PAGE_SIZE(fs)])))

#define spiffs_get_cache_page(fs, c, ix)     ((u8_t*)(&((c)->cpages[(ix) * SPIFFS_CACHE_PAGE_SIZE(fs)])) + sizeof(spiffs_cache_page))
...
typedef s32_t (*spiffs_visitor_f)(spiffs* fs, spiffs_obj_id id, spiffs_block_ix bix, int ix_entry, const void* user_const_p,
                                  void* user_var_p);

#if SPIFFS_CACHE
#define _spiffs_rd(fs, op, fh, addr, len, dst) spiffs_phys_rd((fs), (op), (fh), (addr), (len), (dst))
#define _spiffs_wr(fs, op, fh, addr, len, src) spiffs_phys_wr((fs), (op), (fh), (addr), (len), (src))
#else
...
void spiffs_cb_object_event(spiffs* fs, spiffs_page_object_ix* objix, int ev, spiffs_obj_id obj_id, spiffs_span_ix spix,
...
#ifdef _SPIFFS_TEST
#define _SPIFFS_MEMCPY(__d, __s, __l)                                                            \
    do {                                                                                         \
