...
s32_t spiffs_gc_quick(spiffs* fs, u16_t max_free_pages)
{
    ...
    while (res == SPIFFS_OK && blocks--) {
        ...
        if (res == SPIFFS_OK && deleted_pages_in_block + free_pages_in_block == SPIFFS_PAGES_PER_BLOCK(fs) - SPIFFS_OBJ_LOOKUP_PAGES(fs)
...
s32_t spiffs_gc_check(spiffs* fs, u32_t len)
{
    s32_t res;
    s32_t free_pages =
        (SPIFFS_PAGES_PER_BLOCK(fs) - SPIFFS_OBJ_LOOKUP_PAGES(fs)) * (fs->block_count - 2) - fs->stats_p_allocated - fs->stats_p_deleted;
    int tries = 0;
    ...
    //    free_pages, fs->stats_p_deleted); return SPIFFS_ERR_FULL;
    //  }
    if ((s32_t)needed_pages > (s32_t)(free_pages + fs->stats_p_deleted)) {
        SPIFFS_GC_DBG("gc_check: full freeblk:"_SPIPRIi
                      " needed:"_SPIPRIi
                      " free:"_SPIPRIi
                      " dele:"_SPIPRIi
                      "\n",
    ...
    do {
        SPIFFS_GC_DBG("\ngc_check #"_SPIPRIi
                      ...
                      (free_pages + fs->stats_p_allocated + fs->stats_p_deleted), len, (u32_t)(free_pages * SPIFFS_DATA_PAGE_SIZE(fs)));
        ...
        fs->cleaning = 0;
        if (res < 0) {
            SPIFFS_GC_DBG("gc_check: cleaning block "_SPIPRIi
                          ", result "_SPIPRIi
                          "\n",
                          cand, res);
        } else {
        ...
        free_pages = (SPIFFS_PAGES_PER_BLOCK(fs) - SPIFFS_OBJ_LOOKUP_PAGES(fs)) * (fs->block_count - 2) - fs->stats_p_allocated
                     - fs->stats_p_deleted;
    ...
    SPIFFS_GC_DBG("gc_check: finished, "_SPIPRIi
...
s32_t spiffs_gc_erase_page_stats(spiffs* fs, spiffs_block_ix bix)
{
    ...
    while (res == SPIFFS_OK && obj_lookup_page < (int)SPIFFS_OBJ_LOOKUP_PAGES(fs)) {
        ...
        res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_READ, 0,
                         bix * SPIFFS_CFG_LOG_BLOCK_SZ(fs) + SPIFFS_PAGE_TO_PADDR(fs, obj_lookup_page), SPIFFS_CFG_LOG_PAGE_SZ(fs),
                         fs->lu_work);
        // check each entry
        while (res == SPIFFS_OK && cur_entry - entry_offset < entries_per_page
               && cur_entry < (int)(SPIFFS_PAGES_PER_BLOCK(fs) - SPIFFS_OBJ_LOOKUP_PAGES(fs))) {
    ...
    SPIFFS_GC_DBG("gc_check: wipe pallo:"_SPIPRIi
                  " pdele:"_SPIPRIi
                  "\n",
                  allo, dele);
    ...
    *block_candidates = cand_blocks;
    ...
    while (res == SPIFFS_OK && blocks--) {
        ...
        if (res == SPIFFS_OK /*&& deleted_pages_in_block > 0*/) {
            ...
            res = _spiffs_rd(fs, SPIFFS_OP_C_READ | SPIFFS_OP_T_OBJ_LU2, 0, SPIFFS_ERASE_COUNT_PADDR(fs, cur_block), sizeof(spiffs_obj_id),
                             (u8_t*)&erase_count);
            SPIFFS_CHECK_RES(res);
            ...
            s32_t score = deleted_pages_in_block * SPIFFS_GC_HEUR_W_DELET + used_pages_in_block * SPIFFS_GC_HEUR_W_USED
                          + erase_age * (fs_crammed ? 0 : SPIFFS_GC_HEUR_W_ERASE_AGE);
            int cand_ix = 0;
            SPIFFS_GC_DBG("gc_check: bix:"_SPIPRIbl
                          " del:"_SPIPRIi
                          " use:"_SPIPRIi
                          " score:"_SPIPRIi
                          "\n",
                ...
                switch (gc.state) {
                    case FIND_OBJ_DATA:
                        ...
                        if (obj_id != SPIFFS_OBJ_ID_DELETED && obj_id != SPIFFS_OBJ_ID_FREE && ((obj_id & SPIFFS_OBJ_ID_IX_FLAG) == 0)) {
                            ...
                            SPIFFS_GC_DBG("gc_clean: FIND_DATA state:"_SPIPRIi
                    ...
                    case MOVE_OBJ_DATA:
                        ...
                        if (obj_id == gc.cur_obj_id) {
                            ...
                            SPIFFS_CHECK_RES(res);
                            SPIFFS_GC_DBG("gc_clean: MOVE_DATA found data page "_SPIPRIid
                                          ":"_SPIPRIsp
                                          " @ "_SPIPRIpg
                                          "\n",
                            ...
                            if (SPIFFS_OBJ_IX_ENTRY_SPAN_IX(fs, p_hdr.span_ix) != gc.cur_objix_spix) {
                                ...
                                if (p_hdr.flags & SPIFFS_PH_FLAG_DELET) {
                                    ...
                                    SPIFFS_GC_DBG("gc_clean: MOVE_DATA move objix "_SPIPRIid
                                                  ":"_SPIPRIsp
                                                  " page "_SPIPRIpg
                                                  " to "_SPIPRIpg
                                                  "\n",
                                    ...
                                    SPIFFS_GC_DBG("gc_clean: MOVE_DATA wipe objix "_SPIPRIid
                                                  ":"_SPIPRIsp
                                                  " page "_SPIPRIpg
                                                  "\n",
                                ...
                                if (gc.cur_objix_spix == 0) {
                                    ...
                                    ((spiffs_page_ix*)((u8_t*)objix_hdr + sizeof(spiffs_page_object_ix_header)))[p_hdr.span_ix] =
                                        new_data_pix;
                                    SPIFFS_GC_DBG("gc_clean: MOVE_DATA wrote page "_SPIPRIpg
                                                  " to objix_hdr entry "_SPIPRIsp
                                                  " in mem\n",
                                                  new_data_pix, (spiffs_span_ix)SPIFFS_OBJ_IX_ENTRY(fs, p_hdr.span_ix));
                                } else {
                                    // update object index page
                                    ((spiffs_page_ix*)((u8_t*)objix + sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(fs, p_hdr.span_ix)] =
                                        new_data_pix;
                                    SPIFFS_GC_DBG("gc_clean: MOVE_DATA wrote page "_SPIPRIpg
                    ...
                    case MOVE_OBJ_IX:
                        ...
                        if (obj_id != SPIFFS_OBJ_ID_DELETED && obj_id != SPIFFS_OBJ_ID_FREE && (obj_id & SPIFFS_OBJ_ID_IX_FLAG)) {
                            ...
                            if (p_hdr.flags & SPIFFS_PH_FLAG_DELET) {
                                ...
                                SPIFFS_GC_DBG("gc_clean: MOVE_OBJIX move objix "_SPIPRIid
                                              ":"_SPIPRIsp
                                              " page "_SPIPRIpg
                                              " to "_SPIPRIpg
                                              "\n",
                                ...
                                SPIFFS_CHECK_RES(res);
                                spiffs_cb_object_event(fs, (spiffs_page_object_ix*)&p_hdr, SPIFFS_EV_IX_MOV, obj_id, p_hdr.span_ix, new_pix,
                                                       0);
                                // move wipes obj_lu, reload it
                                res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_READ, 0,
                                                 bix * SPIFFS_CFG_LOG_BLOCK_SZ(fs) + SPIFFS_PAGE_TO_PADDR(fs, obj_lookup_page),
                                ...
                                // might seem unnecessary as we will erase this block, but
                                // we might get aborted
                                SPIFFS_GC_DBG("gc_clean: MOVE_OBJIX wipe objix "_SPIPRIid
                                              ":"_SPIPRIsp
                                              " page "_SPIPRIpg
                                              "\n",
                                              obj_id, p_hdr.span_ix, cur_pix);
                                res = spiffs_page_delete(fs, cur_pix);
                                if (res == SPIFFS_OK) {
                                    spiffs_cb_object_event(fs, (spiffs_page_object_ix*)0, SPIFFS_EV_IX_DEL, obj_id, p_hdr.span_ix, cur_pix,
                                                           0);
                                }
                            }
                            SPIFFS_CHECK_RES(res);
        ...
        switch (gc.state) {
            ...
            case MOVE_OBJ_DATA: {
                ...
                if (gc.cur_objix_spix == 0) {
                    ...
                    res = spiffs_object_update_index_hdr(fs, 0, gc.cur_obj_id | SPIFFS_OBJ_ID_IX_FLAG, gc.cur_objix_pix, fs->work, 0, 0, 0,
                                                         &new_objix_pix);
                    SPIFFS_GC_DBG("gc_clean: MOVE_DATA store modified objix_hdr page, "_SPIPRIpg
                                  ":"_SPIPRIsp
                                  "\n",
                                  new_objix_pix, 0);
                    SPIFFS_CHECK_RES(res);
                    ...
                    SPIFFS_GC_DBG("gc_clean: MOVE_DATA store modified objix page, "_SPIPRIpg
                                  ":"_SPIPRIsp
                                  "\n",
                                  new_objix_pix, objix->p_hdr.span_ix);
                    SPIFFS_CHECK_RES(res);
                    spiffs_cb_object_event(fs, (spiffs_page_object_ix*)fs->work, SPIFFS_EV_IX_UPD, gc.cur_obj_id, objix->p_hdr.span_ix,
                                           new_objix_pix, 0);
                }
            } break;
            case MOVE_OBJ_IX:
