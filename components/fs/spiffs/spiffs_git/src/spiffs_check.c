...
// destroys fs->lu_work
static s32_t spiffs_object_get_data_page_index_reference(spiffs* fs, spiffs_obj_id obj_id, spiffs_span_ix data_spix, spiffs_page_ix* pix,
...
static s32_t spiffs_rewrite_index(spiffs* fs, spiffs_obj_id obj_id, spiffs_span_ix data_spix, spiffs_page_ix new_data_pix,
                                  spiffs_page_ix objix_pix)
{
    s32_t res;
    spiffs_block_ix bix;
    ...
    // load index
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, SPIFFS_PAGE_TO_PADDR(fs, objix_pix), SPIFFS_CFG_LOG_PAGE_SZ(fs),
                     fs->lu_work);
    SPIFFS_CHECK_RES(res);
    ...
    if ((objix_p_hdr->flags
         & (SPIFFS_PH_FLAG_USED | SPIFFS_PH_FLAG_IXDELE | SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_DELET))
...
static s32_t spiffs_delete_obj_lazy(spiffs* fs, spiffs_obj_id obj_id)
{
    ...
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_READ, 0,
                     SPIFFS_PAGE_TO_PADDR(fs, objix_hdr_pix) + offsetof(spiffs_page_header, flags), sizeof(flags), &flags);
    SPIFFS_CHECK_RES(res);
#endif
...
static s32_t spiffs_lookup_check_validate(spiffs* fs, spiffs_obj_id lu_obj_id, spiffs_page_header* p_hdr, spiffs_page_ix cur_pix,
                                          spiffs_block_ix cur_block, int cur_entry, int* reload_lu)
{
    ...
    if (((lu_obj_id == SPIFFS_OBJ_ID_DELETED) && (p_hdr->flags & SPIFFS_PH_FLAG_DELET))
        ...
        SPIFFS_CHECK_DBG("LU: pix "_SPIPRIpg
        ...
        if (p_hdr->flags & SPIFFS_PH_FLAG_INDEX) {
            ...
            if (res == SPIFFS_ERR_NOT_FOUND) {
                ...
                if (ref_pix == cur_pix) {
                    ...
                    SPIFFS_CHECK_DBG("LU: FIXUP: data page not found elsewhere, rewriting "_SPIPRIpg
                                     " to new page "_SPIPRIpg
                                     "\n",
                                     cur_pix, new_pix);
                    SPIFFS_CHECK_RES(res);
                    *reload_lu = 1;
                    SPIFFS_CHECK_DBG("LU: FIXUP: "_SPIPRIpg
                    ...
                    if (res <= _SPIFFS_ERR_CHECK_FIRST && res > _SPIFFS_ERR_CHECK_LAST) {
                        ...
                        SPIFFS_CHECK_DBG("LU: FIXUP: index bad "_SPIPRIi
                                         ", cannot mend!\n",
                                         res);
            ...
            if (res == SPIFFS_ERR_NOT_FOUND) {
                ...
                if (res == SPIFFS_OK) // ignore other errors
                {
                    ...
                    spiffs_page_ix new_pix;
                    res = spiffs_rewrite_page(fs, cur_pix, p_hdr, &new_pix);
                    SPIFFS_CHECK_DBG("LU: FIXUP: ix page with data not found elsewhere, rewriting "_SPIPRIpg
                                     " to new page "_SPIPRIpg
                                     "\n",
                                     cur_pix, new_pix);
                    SPIFFS_CHECK_RES(res);
                ...
                if (p_hdr->flags & SPIFFS_PH_FLAG_INDEX) {
                    ...
                    if (objix_pix_ph == 0 || objix_pix_lu == 0) {
                        ...
                        if ((objix_pix_lu && data_pix_lu && data_pix_ph && objix_pix_ph == 0)
                            ...
                            SPIFFS_CHECK_DBG("LU: FIXUP: rewrite page "_SPIPRIpg
                                             " as "_SPIPRIid
                                             " to pix "_SPIPRIpg
                                             "\n",
            ...
            *reload_lu = 1;
            ...
            if (res == SPIFFS_ERR_NOT_FOUND) {
                ...
                if (ref_pix != cur_pix) {
                    ...
                    u8_t flags = 0xff;
#if SPIFFS_NO_BLIND_WRITES
                    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_READ, 0,
                                     SPIFFS_PAGE_TO_PADDR(fs, cur_pix) + offsetof(spiffs_page_header, flags), sizeof(flags), &flags);
                    SPIFFS_CHECK_RES(res);
#endif
...
static s32_t spiffs_lookup_check_v(spiffs* fs, spiffs_obj_id obj_id, spiffs_block_ix cur_block, int cur_entry, const void* user_const_p,
                                   void* user_var_p)
{
    (void)user_const_p;
    (void)user_var_p;
    ...
    // load header
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, SPIFFS_PAGE_TO_PADDR(fs, cur_pix), sizeof(spiffs_page_header),
                     (u8_t*)&p_hdr);
    SPIFFS_CHECK_RES(res);
...
static s32_t spiffs_page_consistency_check_i(spiffs* fs)
{
    ...
    while (pix_offset < SPIFFS_PAGES_PER_BLOCK(fs) * fs->block_count) {
                ...
                if ((p_hdr.flags & SPIFFS_PH_FLAG_DELET) && (p_hdr.flags & SPIFFS_PH_FLAG_IXDELE)
                    ...
                    for (i = 0; !restart && i < entries; i++) {
                        ...
                        if ((rpix != (spiffs_page_ix)-1 && rpix > SPIFFS_MAX_PAGES(fs))
                            ...
                            // bad reference
                            SPIFFS_CHECK_DBG("PA: pix "_SPIPRIpg
                                             "x bad pix / LU referenced from page "_SPIPRIpg
                                             "\n",
                                             rpix, cur_pix);
                            // check for data page elsewhere
                            spiffs_page_ix data_pix;
                            res = spiffs_obj_lu_find_id_and_span(fs, objix_p_hdr->obj_id & ~SPIFFS_OBJ_ID_IX_FLAG, data_spix_offset + i, 0,
                                                                 &data_pix);
                            if (res == SPIFFS_ERR_NOT_FOUND) {
                                res = SPIFFS_OK;
                                data_pix = 0;
                            ...
                            SPIFFS_CHECK_DBG("PA: FIXUP: rewriting index pix "_SPIPRIpg
                                             "\n",
                                             cur_pix);
                            res = spiffs_rewrite_index(fs, objix_p_hdr->obj_id | SPIFFS_OBJ_ID_IX_FLAG, data_spix_offset + i, data_pix,
                                                       cur_pix);
                            if (res <= _SPIFFS_ERR_CHECK_FIRST && res > _SPIFFS_ERR_CHECK_LAST) {
                                // index bad also, cannot mend this file
                                SPIFFS_CHECK_DBG("PA: FIXUP: index bad "_SPIPRIi
                            ...
                            res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, SPIFFS_PAGE_TO_PADDR(fs, rpix),
                                             sizeof(spiffs_page_header), (u8_t*)&rp_hdr);
                            SPIFFS_CHECK_RES(res);

                            // cross reference page header check
                            if (rp_hdr.obj_id != (p_hdr.obj_id & ~SPIFFS_OBJ_ID_IX_FLAG) || rp_hdr.span_ix != data_spix_offset + i
                                ...
                                spiffs_page_ix data_pix;
                                res = spiffs_obj_lu_find_id_and_span(fs, p_hdr.obj_id & ~SPIFFS_OBJ_ID_IX_FLAG, data_spix_offset + i, rpix,
                                                                     &data_pix);
                                if (res == SPIFFS_ERR_NOT_FOUND) {
                                    res = SPIFFS_OK;
                                    data_pix = 0;
                                ...
                                if (data_pix == 0) {
                                    ...
                                    SPIFFS_CHECK_DBG("PA: FIXUP: found correct data pix "_SPIPRIpg
                                ...
                                if (fs->work[rpix_byte_ix] & (1 << (rpix_bit_ix + 1))) {
                                    ...
                                    SPIFFS_CHECK_DBG("PA: FIXUP: removing object "_SPIPRIid
                                                     " and page "_SPIPRIpg
                                                     "\n",
            ...
            for (byte_ix = 0; !restart && byte_ix < SPIFFS_CFG_LOG_PAGE_SZ(fs); byte_ix++) {
                for (bit_ix = 0; !restart && bit_ix < 8 / bits; bit_ix++) {
                    ...
                    if (bitmask == 0x1) {
                        ...
                        res = spiffs_object_get_data_page_index_reference(fs, p_hdr.obj_id, p_hdr.span_ix, &rpix, &objix_pix);
                        if (res == SPIFFS_OK) {
                            if (((rpix == (spiffs_page_ix)-1 || rpix > SPIFFS_MAX_PAGES(fs)) || (SPIFFS_IS_LOOKUP_PAGE(fs, rpix)))) {
                                ...
                                SPIFFS_CHECK_DBG("PA: corresponding ref is bad: "_SPIPRIpg
                                ...
                                if (((p_hdr.obj_id & ~SPIFFS_OBJ_ID_IX_FLAG) == rp_hdr.obj_id)
                                    && ((rp_hdr.flags
                                         & (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_USED | SPIFFS_PH_FLAG_FINAL))
                                        == (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_DELET))) {
                                    // pointing to something else valid, just delete this page then
                                    SPIFFS_CHECK_DBG("PA: corresponding ref is good but different: "_SPIPRIpg
                                    ...
                                    if (rpix != cur_pix) {
                                        SPIFFS_CHECK_DBG("PA: corresponding ref is weird: "_SPIPRIpg
                        ...
                        if (rewrite_ix_to_this) {
                            ...
                            SPIFFS_CHECK_DBG("PA: FIXUP: rewrite index id "_SPIPRIid
                                             " data spix "_SPIPRIsp
        ...
        SPIFFS_CHECK_DBG("PA: processed "_SPIPRIpg
                         ", restart "_SPIPRIi
                         "\n",
                         pix_offset, restart);
        // next page range
...
static s32_t spiffs_object_index_consistency_check_v(spiffs* fs, spiffs_obj_id obj_id, spiffs_block_ix cur_block, int cur_entry,
                                                     const void* user_const_p, void* user_var_p)
{
    ...
    if (obj_id != SPIFFS_OBJ_ID_FREE && obj_id != SPIFFS_OBJ_ID_DELETED && (obj_id & SPIFFS_OBJ_ID_IX_FLAG)) {
        ...
        if (p_hdr.span_ix == 0
            ...
            SPIFFS_CHECK_DBG("IX: pix "_SPIPRIpg
                             ", obj id:"_SPIPRIid
                             " spix:"_SPIPRIsp
        ...
        } else // span index
        {
            ...
            if (delete) {
                SPIFFS_CHECK_DBG("IX: FIXUP: pix "_SPIPRIpg
                                 ", obj id:"_SPIPRIid
                                 " spix:"_SPIPRIsp
...
s32_t spiffs_object_index_consistency_check(spiffs* fs)
{
    ...
    res = spiffs_obj_lu_find_entry_visitor(fs, 0, 0, 0, 0, spiffs_object_index_consistency_check_v, 0, &obj_id_log_ix, 0, 0);
    if (res == SPIFFS_VIS_END) {
        res = SPIFFS_OK;
    }
    if (res != SPIFFS_OK) {
