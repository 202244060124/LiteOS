...
static s32_t spiffs_page_data_check(spiffs* fs, spiffs_fd* fd, spiffs_page_ix pix, spiffs_span_ix spix)
{
    ...
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_READ, fd->file_nbr, SPIFFS_PAGE_TO_PADDR(fs, pix), sizeof(spiffs_page_header),
                     (u8_t*)&ph);
...
#if SPIFFS_PAGE_CHECK
    spiffs_page_header ph;
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_READ, fd->file_nbr, SPIFFS_PAGE_TO_PADDR(fs, pix), sizeof(spiffs_page_header),
                     (u8_t*)&ph);
    SPIFFS_CHECK_RES(res);
...
s32_t spiffs_obj_lu_find_entry_visitor(spiffs* fs, spiffs_block_ix starting_block, int starting_lu_entry, u8_t flags, spiffs_obj_id obj_id,
                                       spiffs_visitor_f v, const void* user_const_p, void* user_var_p, spiffs_block_ix* block_ix,
                                       int* lu_entry)
{
    s32_t res = SPIFFS_OK;
    ...
    while (res == SPIFFS_OK && entry_count > 0) {
        ...
        while (res == SPIFFS_OK && obj_lookup_page < (int)SPIFFS_OBJ_LOOKUP_PAGES(fs)) {
            ...
            res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_READ, 0, cur_block_addr + SPIFFS_PAGE_TO_PADDR(fs, obj_lookup_page),
                   ...
                   cur_entry < (int)SPIFFS_OBJ_LOOKUP_MAX_ENTRIES(fs))                // for last obj lookup page
            {
                if ((flags & SPIFFS_VIS_CHECK_ID) == 0 || obj_lu_buf[cur_entry - entry_offset] == obj_id) {
                    ...
                    if (lu_entry)
                        *lu_entry = cur_entry;
                    if (v) {
                        res = v(fs, (flags & SPIFFS_VIS_CHECK_PH) ? obj_id : obj_lu_buf[cur_entry - entry_offset], cur_block, cur_entry,
                                user_const_p, user_var_p);
...
s32_t spiffs_erase_block(spiffs* fs, spiffs_block_ix bix)
{
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_C_WRTHRU | SPIFFS_OP_T_OBJ_LU2, 0, SPIFFS_ERASE_COUNT_PADDR(fs, bix), sizeof(spiffs_obj_id),
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_C_WRTHRU | SPIFFS_OP_T_OBJ_LU2, 0, SPIFFS_MAGIC_PADDR(fs, bix), sizeof(spiffs_obj_id), (u8_t*)&magic);
    SPIFFS_CHECK_RES(res);
#endif
...
static s32_t spiffs_obj_lu_scan_v(spiffs* fs, spiffs_obj_id obj_id, spiffs_block_ix bix, int ix_entry, const void* user_const_p,
                                  void* user_var_p)
{
    (void)bix;
    (void)user_const_p;
...
s32_t spiffs_obj_lu_scan(spiffs* fs)
{
    ...
    while (bix < fs->block_count) {
        ...
        res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, SPIFFS_ERASE_COUNT_PADDR(fs, bix), sizeof(spiffs_obj_id),
                         (u8_t*)&erase_count);
        SPIFFS_CHECK_RES(res);
...
{
    s32_t res =
        spiffs_obj_lu_find_entry_visitor(fs, starting_block, starting_lu_entry, SPIFFS_VIS_CHECK_ID, obj_id, 0, 0, 0, block_ix, lu_entry);
    if (res == SPIFFS_VIS_END) {
...
static s32_t spiffs_obj_lu_find_id_and_span_v(spiffs* fs, spiffs_obj_id obj_id, spiffs_block_ix bix, int ix_entry, const void* user_const_p,
                                              void* user_var_p)
{
    s32_t res;
    spiffs_page_header ph;
    spiffs_page_ix pix = SPIFFS_OBJ_LOOKUP_ENTRY_TO_PIX(fs, bix, ix_entry);
    res = _spiffs_rd(fs, 0, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, SPIFFS_PAGE_TO_PADDR(fs, pix), sizeof(spiffs_page_header), (u8_t*)&ph);
    SPIFFS_CHECK_RES(res);
...
// Iterate over object lookup pages in each block until a given object id entry is found
s32_t spiffs_obj_lu_find_id_and_span(spiffs* fs, spiffs_obj_id obj_id, spiffs_span_ix spix, spiffs_page_ix exclusion_pix,
                                     spiffs_page_ix* pix)
{
    s32_t res;
    spiffs_block_ix bix;
    ...
    res = spiffs_obj_lu_find_entry_visitor(fs, fs->cursor_block_ix, fs->cursor_obj_lu_entry, SPIFFS_VIS_CHECK_ID, obj_id,
                                           spiffs_obj_lu_find_id_and_span_v, exclusion_pix ? &exclusion_pix : 0, &spix, &bix, &entry);
...
// Iterate over object lookup pages in each block until a given object id entry is found
s32_t spiffs_obj_lu_find_id_and_span_by_phdr(spiffs* fs, spiffs_obj_id obj_id, spiffs_span_ix spix, spiffs_page_ix exclusion_pix,
                                             spiffs_page_ix* pix)
{
    s32_t res;
    spiffs_block_ix bix;
    ...
    res = spiffs_obj_lu_find_entry_visitor(fs, fs->cursor_block_ix, fs->cursor_obj_lu_entry, SPIFFS_VIS_CHECK_PH, obj_id,
                                           spiffs_obj_lu_find_id_and_span_v, exclusion_pix ? &exclusion_pix : 0, &spix, &bix, &entry);
...
static void spiffs_update_ix_map(spiffs* fs, spiffs_fd* fd, spiffs_span_ix objix_spix, spiffs_page_object_ix* objix)
{
#if SPIFFS_SINGLETON
    (void)fs;
#endif
    ...
    spiffs_span_ix objix_data_spix_start = SPIFFS_DATA_SPAN_IX_FOR_OBJ_IX_SPAN_IX(fs, objix_spix);
    spiffs_span_ix objix_data_spix_end = objix_data_spix_start + (objix_spix == 0 ? SPIFFS_OBJ_HDR_IX_LEN(fs) : SPIFFS_OBJ_IX_LEN(fs));
    ...
    while (map_spix < map_spix_end) {
        ...
        SPIFFS_DBG("map "_SPIPRIid
                   ...
                   " to pix "_SPIPRIpg
                   "\n",
                   fd->obj_id, map_spix - map->start_spix, map->start_spix, map->end_spix, objix->p_hdr.span_ix, objix_data_pix);
...
static s32_t spiffs_populate_ix_map_v(spiffs* fs, spiffs_obj_id obj_id, spiffs_block_ix bix, int ix_entry, const void* user_const_p,
                                      void* user_var_p)
{
    (void)user_const_p;
    s32_t res;
    ...
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, SPIFFS_PAGE_TO_PADDR(fs, pix), sizeof(spiffs_page_object_ix),
                     (u8_t*)objix);
    SPIFFS_CHECK_RES(res);
    ...
    if ((objix->p_hdr.flags & (SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_IXDELE))
        ...
        && objix->p_hdr.span_ix >= state->map_objix_start_spix && objix->p_hdr.span_ix <= state->map_objix_end_spix) {
        // ok, load rest of object index
        res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, SPIFFS_PAGE_TO_PADDR(fs, pix) + sizeof(spiffs_page_object_ix),
                         SPIFFS_CFG_LOG_PAGE_SZ(fs) - sizeof(spiffs_page_object_ix), (u8_t*)objix + sizeof(spiffs_page_object_ix));
        SPIFFS_CHECK_RES(res);
        ...
        SPIFFS_DBG("map "_SPIPRIid
                   ...
                   "\n",
                   state->fd->obj_id, state->fd->ix_map->start_spix, state->fd->ix_map->end_spix, state->remaining_objix_pages_to_visit);
    }
...
s32_t spiffs_populate_ix_map(spiffs* fs, spiffs_fd* fd, u32_t vec_entry_start, u32_t vec_entry_end)
{
    ...
    state.remaining_objix_pages_to_visit = state.map_objix_end_spix - state.map_objix_start_spix + 1;
    state.fd = fd;

    res = spiffs_obj_lu_find_entry_visitor(fs, SPIFFS_BLOCK_FOR_PAGE(fs, fd->objix_hdr_pix),
                                           SPIFFS_OBJ_LOOKUP_ENTRY_FOR_PAGE(fs, fd->objix_hdr_pix), SPIFFS_VIS_CHECK_ID,
                                           fd->obj_id | SPIFFS_OBJ_ID_IX_FLAG, spiffs_populate_ix_map_v, 0, &state, 0, 0);
...
s32_t spiffs_page_allocate_data(spiffs* fs, spiffs_obj_id obj_id, spiffs_page_header* ph, u8_t* data, u32_t len, u32_t page_offs,
                                u8_t finalize, spiffs_page_ix* pix)
{
    s32_t res = SPIFFS_OK;
    spiffs_block_ix bix;
    int entry;
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_UPDT, 0, SPIFFS_BLOCK_TO_PADDR(fs, bix) + entry * sizeof(spiffs_obj_id),
    ...
    if (data) {
        res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, 0,
                         SPIFFS_OBJ_LOOKUP_ENTRY_TO_PADDR(fs, bix, entry) + sizeof(spiffs_page_header) + page_offs, len, data);
        SPIFFS_CHECK_RES(res);
    }
    ...
    if (finalize && (ph->flags & SPIFFS_PH_FLAG_FINAL)) {
        ...
        res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, 0,
                         SPIFFS_OBJ_LOOKUP_ENTRY_TO_PADDR(fs, bix, entry) + offsetof(spiffs_page_header, flags), sizeof(u8_t),
                         (u8_t*)&ph->flags);
        SPIFFS_CHECK_RES(res);
    }
                       ...
                       spiffs_page_ix src_pix, spiffs_page_ix* dst_pix)
{
    ...
    if (page_data) {
        ...
        res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, 0, SPIFFS_PAGE_TO_PADDR(fs, free_pix), SPIFFS_CFG_LOG_PAGE_SZ(fs),
                         page_data);
    } else {
        // copy page data
    ...
    if (was_final) {
        ...
        res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, fh,
                         SPIFFS_PAGE_TO_PADDR(fs, free_pix) + offsetof(spiffs_page_header, flags), sizeof(u8_t), (u8_t*)&p_hdr->flags);
        SPIFFS_CHECK_RES(res);
    }
    // mark source deleted
...
s32_t spiffs_page_delete(spiffs* fs, spiffs_page_ix pix)
{
    ...
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_READ, 0, SPIFFS_PAGE_TO_PADDR(fs, pix) + offsetof(spiffs_page_header, flags),
                     sizeof(flags), &flags);
    SPIFFS_CHECK_RES(res);
#endif
    flags &= ~(SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_USED);
    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_DELE, 0, SPIFFS_PAGE_TO_PADDR(fs, pix) + offsetof(spiffs_page_header, flags),
                     sizeof(flags), &flags);
...
s32_t spiffs_object_create(spiffs* fs, spiffs_obj_id obj_id, const u8_t name[], const u8_t meta[], spiffs_obj_type type,
                           spiffs_page_ix* objix_hdr_pix)
{
    ...
    res = spiffs_obj_lu_find_free(fs, fs->free_cursor_block_ix, fs->free_cursor_obj_lu_entry, &bix, &entry);
    SPIFFS_CHECK_RES(res);
    SPIFFS_DBG("create: found free page @ "_SPIPRIpg
               " bix:"_SPIPRIbl
               " entry:"_SPIPRIsp
               "\n",
    ...
    SPIFFS_CHECK_RES(res);
    spiffs_cb_object_event(fs, (spiffs_page_object_ix*)&oix_hdr, SPIFFS_EV_IX_NEW, obj_id, 0,
                           SPIFFS_OBJ_LOOKUP_ENTRY_TO_PIX(fs, bix, entry), SPIFFS_UNDEFINED_LEN);
...
s32_t spiffs_object_update_index_hdr(spiffs* fs, spiffs_fd* fd, spiffs_obj_id obj_id, spiffs_page_ix objix_hdr_pix,
                                     u8_t* new_objix_hdr_data, const u8_t name[], const u8_t meta[], u32_t size, spiffs_page_ix* new_pix)
{
    s32_t res = SPIFFS_OK;
    ...
    if (new_objix_hdr_data) {
        ...
        res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_READ, fd->file_nbr, SPIFFS_PAGE_TO_PADDR(fs, objix_hdr_pix),
    ...
    if (res == SPIFFS_OK) {
        ...
        spiffs_cb_object_event(fs, (spiffs_page_object_ix*)objix_hdr, new_objix_hdr_data ? SPIFFS_EV_IX_UPD : SPIFFS_EV_IX_UPD_HDR, obj_id,
                               objix_hdr->p_hdr.span_ix, new_objix_hdr_pix, objix_hdr->size);
        if (fd)
...
void spiffs_cb_object_event(spiffs* fs, spiffs_page_object_ix* objix, int ev, spiffs_obj_id obj_id_raw, spiffs_span_ix spix,
                            spiffs_page_ix new_pix, u32_t new_size)
{
    ...
    SPIFFS_DBG("       CALLBACK  %s obj_id:"_SPIPRIid
               " spix:"_SPIPRIsp
               " npix:"_SPIPRIpg
               " nsz:"_SPIPRIi
               "\n",
               (const char*[]){"UPD", "NEW", "DEL", "MOV", "HUP", "???"}[MIN(ev, 5)], obj_id_raw, spix, new_pix, new_size);
            ...
            if (ev != SPIFFS_EV_IX_DEL) {
...
#endif
                SPIFFS_DBG("       callback: setting fd "_SPIPRIfd
                           ":"_SPIPRIid
                           "(fdoffs:"_SPIPRIi
                           " offs:"_SPIPRIi
                ...
                if (new_size != 0) {
                    ...
                    if (cur_fd->cache_page && cur_fd->cache_page->offset > act_new_size + 1) {
                        SPIFFS_CACHE_DBG("CACHE_DROP: file trunced, dropping cache page "_SPIPRIi
                                         ", no writeback\n",
                ...
                if (cur_fd->file_nbr && cur_fd->cache_page) {
                    SPIFFS_CACHE_DBG("CACHE_DROP: file deleted, dropping cache page "_SPIPRIi
                                     ", no writeback\n",
                                     cur_fd->cache_page->ix);
                    spiffs_cache_fd_release(fs, cur_fd->cache_page);
                }
#endif
                SPIFFS_DBG("       callback: release fd "_SPIPRIfd
                           ":"_SPIPRIid
                           " span:"_SPIPRIsp
    ...
    if (ev == SPIFFS_EV_IX_UPD || ev == SPIFFS_EV_IX_NEW) {
        for (i = 0; i < fs->fd_count; i++) {
            ...
            if (cur_fd->file_nbr == 0 || cur_fd->ix_map == 0 || (cur_fd->obj_id & ~SPIFFS_OBJ_ID_IX_FLAG) != obj_id)
                continue;
            SPIFFS_DBG("       callback: map ix update fd "_SPIPRIfd
                       ":"_SPIPRIid
                       " span:"_SPIPRIsp
                       "\n",
...
s32_t spiffs_object_append(spiffs_fd* fd, u32_t offset, u8_t* data, u32_t len)
{
    ...
    while (res == SPIFFS_OK && written < len) {
        ...
        if (cur_objix_spix != prev_objix_spix) {
            ...
            if (written > 0) {
                ...
                if (prev_objix_spix == 0) {
                    ...
                    if (offset == 0) {
                        ...
                        SPIFFS_CHECK_RES(res);
                        res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_UPDT, fd->file_nbr, SPIFFS_PAGE_TO_PADDR(fs, cur_objix_pix),
                                         SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->work);
                        SPIFFS_CHECK_RES(res);
                    } else {
                        // was a nonempty object, update to new page
                        res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, fs->work, 0, 0, offset + written,
                                                             &new_objix_hdr_page);
                        SPIFFS_CHECK_RES(res);
                        SPIFFS_DBG("append: "_SPIPRIid
                    ...
                    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_UPDT, fd->file_nbr, SPIFFS_PAGE_TO_PADDR(fs, cur_objix_pix),
                                     SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->work);
                    SPIFFS_CHECK_RES(res);
                    spiffs_cb_object_event(fs, (spiffs_page_object_ix*)fs->work, SPIFFS_EV_IX_UPD, fd->obj_id, objix->p_hdr.span_ix,
                                           cur_objix_pix, 0);
                    // update length in object index header page
                    res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, 0, 0, 0, offset + written,
                                                         &new_objix_hdr_page);
                    SPIFFS_CHECK_RES(res);
                    SPIFFS_DBG("append: "_SPIPRIid
            ...
            if (cur_objix_spix == 0) {
                ...
                res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_READ, fd->file_nbr, SPIFFS_PAGE_TO_PADDR(fs, cur_objix_pix),
                                 SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->work);
                SPIFFS_CHECK_RES(res);
                ...
                if (written > 0 || cur_objix_spix > len_objix_spix) {
                    ...
                    res = spiffs_page_allocate_data(fs, fd->obj_id | SPIFFS_OBJ_ID_IX_FLAG, &p_hdr, 0, 0, 0, 1, &cur_objix_pix);
                    SPIFFS_CHECK_RES(res);
                    // quick "load" of new object index page
                    memset(fs->work, 0xff, SPIFFS_CFG_LOG_PAGE_SZ(fs));
                    _SPIFFS_MEMCPY(fs->work, &p_hdr, sizeof(spiffs_page_header));
                    spiffs_cb_object_event(fs, (spiffs_page_object_ix*)fs->work, SPIFFS_EV_IX_NEW, fd->obj_id, cur_objix_spix,
                                           cur_objix_pix, 0);
                    SPIFFS_DBG("append: "_SPIPRIid
        ...
        if (page_offs == 0) {
            ...
            res = spiffs_page_allocate_data(fs, fd->obj_id & ~SPIFFS_OBJ_ID_IX_FLAG, &p_hdr, &data[written], to_write, page_offs, 1,
                                            &data_page);
            SPIFFS_DBG("append: "_SPIPRIid
            ...
            res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, fd->file_nbr,
                             SPIFFS_PAGE_TO_PADDR(fs, data_page) + sizeof(spiffs_page_header) + page_offs, to_write, &data[written]);
            SPIFFS_DBG("append: "_SPIPRIid
        ...
        if (cur_objix_spix == 0) {
            ...
            SPIFFS_DBG("append: "_SPIPRIid
                       ...
                       " in mem\n",
                       fd->obj_id, data_page, (spiffs_span_ix)SPIFFS_OBJ_IX_ENTRY(fs, data_spix));
        }
    ...
    if (cur_objix_spix != 0) {
        ...
        res2 = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_UPDT, fd->file_nbr, SPIFFS_PAGE_TO_PADDR(fs, cur_objix_pix),
                          SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->work);
        SPIFFS_CHECK_RES(res2);
        spiffs_cb_object_event(fs, (spiffs_page_object_ix*)fs->work, SPIFFS_EV_IX_UPD, fd->obj_id, objix->p_hdr.span_ix, cur_objix_pix, 0);

        // update size in object header index page
        res2 = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, 0, 0, 0, offset + written, &new_objix_hdr_page);
        SPIFFS_DBG("append: "_SPIPRIid
        ...
        if (offset == 0) {
            ...
            SPIFFS_CHECK_RES(res2);
            // callback on object index update
            spiffs_cb_object_event(fs, (spiffs_page_object_ix*)fs->work, SPIFFS_EV_IX_UPD_HDR, fd->obj_id, objix_hdr->p_hdr.span_ix,
                                   cur_objix_pix, objix_hdr->size);
        } else {
            // modifying object index header page, update size and make new copy
            res2 = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, fs->work, 0, 0, offset + written,
                                                  &new_objix_hdr_page);
            SPIFFS_DBG("append: "_SPIPRIid
...
s32_t spiffs_object_modify(spiffs_fd* fd, u32_t offset, u8_t* data, u32_t len)
{
    ...
    while (res == SPIFFS_OK && written < len) {
        ...
        if (cur_objix_spix != prev_objix_spix) {
            ...
            if (written > 0) {
                ...
                if (prev_objix_spix == 0) {
                    ...
                    res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, fs->work, 0, 0, 0, &new_objix_hdr_pix);
                    SPIFFS_DBG("modify: store modified objix_hdr page, "_SPIPRIpg
                               ":"_SPIPRIsp
                               ", written "_SPIPRIi
                               "\n",
                    ...
                    SPIFFS_DBG("modify: store previous modified objix page, "_SPIPRIid
                               ":"_SPIPRIsp
                               ", written "_SPIPRIi
                               "\n",
                               new_objix_pix, objix->p_hdr.span_ix, written);
                    SPIFFS_CHECK_RES(res);
                    spiffs_cb_object_event(fs, (spiffs_page_object_ix*)objix, SPIFFS_EV_IX_UPD, fd->obj_id, objix->p_hdr.span_ix,
                                           new_objix_pix, 0);
                }
            }
            ...
            if (cur_objix_spix == 0) {
                ...
                SPIFFS_DBG("modify: load objixhdr page "_SPIPRIpg
                           ":"_SPIPRIsp
                           "\n",
        ...
        if (page_offs == 0 && to_write == SPIFFS_DATA_PAGE_SIZE(fs)) {
            ...
            res = spiffs_page_allocate_data(fs, fd->obj_id & ~SPIFFS_OBJ_ID_IX_FLAG, &p_hdr, &data[written], to_write, page_offs, 1,
                                            &data_pix);
            SPIFFS_DBG("modify: store new data page, "_SPIPRIpg
                       ":"_SPIPRIsp
                       " offset:"_SPIPRIi
                       ", len "_SPIPRIi
                       ", written "_SPIPRIi
            ...
            p_hdr.flags &= ~SPIFFS_PH_FLAG_FINAL;
            res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, fd->file_nbr,
                             SPIFFS_PAGE_TO_PADDR(fs, data_pix) + offsetof(spiffs_page_header, flags), sizeof(u8_t), (u8_t*)&p_hdr.flags);
            if (res != SPIFFS_OK)
                break;

            SPIFFS_DBG("modify: store to existing data page, src:"_SPIPRIpg
                       ", dst:"_SPIPRIpg
                       ":"_SPIPRIsp
                       " offset:"_SPIPRIi
                       ", len "_SPIPRIi
        ...
        if (cur_objix_spix == 0) {
            ...
            SPIFFS_DBG("modify: wrote page "_SPIPRIpg
                       " to objix_hdr entry "_SPIPRIsp
                       " in mem\n",
                       data_pix, data_spix);
        } else {
    ...
    if (cur_objix_spix != 0) {
        ...
        SPIFFS_DBG("modify: store modified objix page, "_SPIPRIpg
                   ":"_SPIPRIsp
                   ", written "_SPIPRIi
                   "\n",
        ...
        fd->cursor_objix_spix = cur_objix_spix;
        SPIFFS_CHECK_RES(res2);
        spiffs_cb_object_event(fs, (spiffs_page_object_ix*)objix, SPIFFS_EV_IX_UPD, fd->obj_id, objix->p_hdr.span_ix, new_objix_pix, 0);

    } else {
        // wrote within object index header page
        res2 = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, fs->work, 0, 0, 0, &new_objix_hdr_pix);
        SPIFFS_DBG("modify: store modified objix_hdr page, "_SPIPRIpg
                   ":"_SPIPRIsp
                   ", written "_SPIPRIi
                   "\n",
...
static s32_t spiffs_object_find_object_index_header_by_name_v(spiffs* fs, spiffs_obj_id obj_id, spiffs_block_ix bix, int ix_entry,
                                                              const void* user_const_p, void* user_var_p)
{
    ...
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, SPIFFS_PAGE_TO_PADDR(fs, pix), sizeof(spiffs_page_object_ix_header),
                     (u8_t*)&objix_hdr);
    SPIFFS_CHECK_RES(res);
...
s32_t spiffs_object_truncate(spiffs_fd* fd, u32_t new_size, u8_t remove_full)
{
    s32_t res = SPIFFS_OK;
    spiffs* fs = fd->fs;
    ...
    if (remove_full && new_size == 0) {
        ...
        res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_UPDT, fd->file_nbr,
                         SPIFFS_PAGE_TO_PADDR(fs, fd->objix_hdr_pix) + offsetof(spiffs_page_header, flags), sizeof(u8_t), (u8_t*)&flags);
        SPIFFS_CHECK_RES(res);
    }
    ...
    while (cur_size > new_size) {
        ...
        if (prev_objix_spix != cur_objix_spix) {
            if (prev_objix_spix != (spiffs_span_ix)-1) {
                ...
                SPIFFS_DBG("truncate: delete objix page "_SPIPRIpg
                           ":"_SPIPRIsp
                           "\n",
                ...
                SPIFFS_CHECK_RES(res);
                spiffs_cb_object_event(fs, (spiffs_page_object_ix*)0, SPIFFS_EV_IX_DEL, fd->obj_id, objix->p_hdr.span_ix, objix_pix, 0);
                if (prev_objix_spix > 0) {
                    ...
                    if (remove_full == 0) {
                        SPIFFS_DBG("truncate: update objix hdr page "_SPIPRIpg
                                   ":"_SPIPRIsp
                                   " to size "_SPIPRIi
                                   "\n",
                                   fd->objix_hdr_pix, prev_objix_spix, cur_size);
                        res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, 0, 0, 0, cur_size, &new_objix_hdr_pix);
                        SPIFFS_CHECK_RES(res);
                    }
                    fd->size = cur_size;
                }
            ...
            SPIFFS_DBG("truncate: load objix page "_SPIPRIpg
                       ":"_SPIPRIsp
                       " for data spix:"_SPIPRIsp
                       "\n",
                       objix_pix, cur_objix_spix, data_spix);
            res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_READ, fd->file_nbr, SPIFFS_PAGE_TO_PADDR(fs, objix_pix),
                             SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->work);
            SPIFFS_CHECK_RES(res);
            SPIFFS_VALIDATE_OBJIX(objix_hdr->p_hdr, fd->obj_id, cur_objix_spix);
        ...
        if (new_size == 0 || remove_full || cur_size - new_size >= SPIFFS_DATA_PAGE_SIZE(fs)) {
            ...
            fd->size = cur_size;
            fd->offset = cur_size;
            SPIFFS_DBG("truncate: delete data page "_SPIPRIpg
                       " for data spix:"_SPIPRIsp
                       ", cur_size:"_SPIPRIi
                       "\n",
            ...
            SPIFFS_DBG("truncate: delete "_SPIPRIi
            ...
            if (cur_objix_spix == 0) {
                ...
                SPIFFS_DBG("truncate: wrote page "_SPIPRIpg
                           " to objix_hdr entry "_SPIPRIsp
                           " in mem\n",
    ...
    if (cur_objix_spix == 0) {
        ...
        if (cur_size == 0) {
            if (remove_full) {
                ...
                res = spiffs_page_delete(fs, objix_pix);
                SPIFFS_CHECK_RES(res);
                spiffs_cb_object_event(fs, (spiffs_page_object_ix*)0, SPIFFS_EV_IX_DEL, fd->obj_id, 0, objix_pix, 0);
            } else {
                ...
                res =
                    spiffs_object_update_index_hdr(fs, fd, fd->obj_id, objix_pix, fs->work, 0, 0, SPIFFS_UNDEFINED_LEN, &new_objix_hdr_pix);
                SPIFFS_CHECK_RES(res);
            }
        } else {
            // update object index header page
            SPIFFS_DBG("truncate: update object index header page with indices and size\n");
            res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, objix_pix, fs->work, 0, 0, cur_size, &new_objix_hdr_pix);
            SPIFFS_CHECK_RES(res);
        }
    } else {
        ...
        SPIFFS_CHECK_RES(res);
        spiffs_cb_object_event(fs, (spiffs_page_object_ix*)objix_hdr, SPIFFS_EV_IX_UPD, fd->obj_id, objix->p_hdr.span_ix, new_objix_pix, 0);
        SPIFFS_DBG("truncate: store modified objix page, "_SPIPRIpg
                   ":"_SPIPRIsp
                   "\n",
...
s32_t spiffs_object_read(spiffs_fd* fd, u32_t offset, u32_t len, u8_t* dst)
{
    ...
    while (cur_offset < offset + len) {
        ...
        if (fd->ix_map && data_spix >= fd->ix_map->start_spix && data_spix <= fd->ix_map->end_spix
            ...
            if (prev_objix_spix != cur_objix_spix) {
                ...
                if (cur_objix_spix == 0) {
                    ...
                    SPIFFS_DBG("read: find objix "_SPIPRIid
                               ":"_SPIPRIsp
                               "\n",
                               fd->obj_id, cur_objix_spix);
                    if (fd->cursor_objix_spix == cur_objix_spix) {
                ...
                SPIFFS_DBG("read: load objix page "_SPIPRIpg
                           ":"_SPIPRIsp
                           " for data spix:"_SPIPRIsp
                           "\n",
                           objix_pix, cur_objix_spix, data_spix);
        ...
        SPIFFS_DBG("read: offset:"_SPIPRIi
                   " rd:"_SPIPRIi
                   " data spix:"_SPIPRIsp
                                                      ...
                                                      const void* user_const_p, void* user_var_p)
{
    ...
    if (id != SPIFFS_OBJ_ID_FREE && id != SPIFFS_OBJ_ID_DELETED && (id & SPIFFS_OBJ_ID_IX_FLAG)) {
        ...
        res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, SPIFFS_OBJ_LOOKUP_ENTRY_TO_PADDR(fs, bix, ix_entry),
                         sizeof(spiffs_page_object_ix_header), (u8_t*)&objix_hdr);
        if (res == SPIFFS_OK && objix_hdr.p_hdr.span_ix == 0
            && ((objix_hdr.p_hdr.flags & (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_DELET)) == (SPIFFS_PH_FLAG_DELET))) {
...
s32_t spiffs_obj_lu_find_free_obj_id(spiffs* fs, spiffs_obj_id* obj_id, const u8_t* conflicting_name)
{
    ...
    while (res == SPIFFS_OK && free_obj_id == SPIFFS_OBJ_ID_FREE) {
        if (state.max_obj_id - state.min_obj_id <= (spiffs_obj_id)SPIFFS_CFG_LOG_PAGE_SZ(fs) * 8) {
            ...
            u32_t i, j;
            SPIFFS_DBG("free_obj_id: BITM min:"_SPIPRIid
                       " max:"_SPIPRIid
                       "\n",
                       state.min_obj_id, state.max_obj_id);

            memset(fs->work, 0, SPIFFS_CFG_LOG_PAGE_SZ(fs));
            res = spiffs_obj_lu_find_entry_visitor(fs, 0, 0, 0, 0, spiffs_obj_lu_find_free_obj_id_bitmap_v, conflicting_name,
            ...
            if (state.compaction != 0) {
                ...
                SPIFFS_DBG("free_obj_id: COMP select index:"_SPIPRIi
                           " min_count:"_SPIPRIi
                           " min:"_SPIPRIid
                           " max:"_SPIPRIid
                           " compact:"_SPIPRIi
                ...
                if (min_count == 0) {
                    ...
                    *obj_id = min_i * state.compaction + state.min_obj_id;
                    ...
                    SPIFFS_DBG("free_obj_id: COMP SEL chunk:"_SPIPRIi
                               " min:"_SPIPRIid
                               " -> "_SPIPRIid
                               "\n",
            ...
            SPIFFS_DBG("free_obj_id: COMP min:"_SPIPRIid
                       " max:"_SPIPRIid
                       " compact:"_SPIPRIi
                       "\n",
                       state.min_obj_id, state.max_obj_id, state.compaction);
...
#if SPIFFS_TEMPORAL_FD_CACHE
void spiffs_fd_temporal_cache_rehash(spiffs* fs, const char* old_path, const char* new_path)
{
    u32_t i;
