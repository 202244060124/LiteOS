...
LV_ATTRIBUTE_FAST_MEM static void draw_bg(const lv_area_t* coords, const lv_area_t* clip, lv_draw_rect_dsc_t* dsc)
{
    ...
    if (other_mask_cnt)
        simple_mode = false;
    else if (dsc->bg_grad_dir == LV_GRAD_DIR_HOR)
        simple_mode = false;
    ...
    if (simple_mode && rout == 0 && (dsc->bg_grad_dir == LV_GRAD_DIR_NONE)) {
        _lv_blend_fill(clip, &coords_bg, dsc->bg_color, NULL, LV_DRAW_MASK_RES_FULL_COVER, opa, dsc->bg_blend_mode);
    }
    /*More complex case: there is a radius, gradient or other mask.*/
        ...
        for (h = draw_area.y1; h <= draw_area.y2; h++) {
            ...
            if (y > coords_bg.y1 + rout + 1 && y < coords_bg.y2 - rout - 1) {
            ...
            if (simple_mode && split && (y < coords_bg.y1 + rout + 1 || y > coords_bg.y2 - rout - 1)) {
                /*Left part*/
                ...
                _lv_blend_fill(clip, &fill_area2, grad_color, mask_buf, mask_res, opa, dsc->bg_blend_mode);

                /*Center part*/
                if (dsc->bg_grad_dir == LV_GRAD_DIR_VER) {
                    fill_area2.x1 = coords_bg.x1 + rout;
                    fill_area2.x2 = coords_bg.x2 - rout;
                    _lv_blend_fill(clip, &fill_area2, grad_color, NULL, LV_DRAW_MASK_RES_FULL_COVER, opa, dsc->bg_blend_mode);
                }
                ...
                if (mask_ofs < 0)
                    mask_ofs = 0;
                _lv_blend_fill(clip, &fill_area2, grad_color, mask_buf + mask_ofs, mask_res, opa, dsc->bg_blend_mode);
                ...
                if (dsc->bg_grad_dir == LV_GRAD_DIR_HOR) {
                    ...
                    _lv_blend_fill(clip, &fill_area, grad_color, mask_buf, mask_res, opa, dsc->bg_blend_mode);
        ...
        if (dsc->bg_grad_dir == LV_GRAD_DIR_NONE && other_mask_cnt == 0 && split) {
            ...
            _lv_blend_fill(clip, &fill_area, dsc->bg_color, NULL, LV_DRAW_MASK_RES_FULL_COVER, opa, dsc->bg_blend_mode);
...
LV_ATTRIBUTE_FAST_MEM static void draw_border(const lv_area_t* coords, const lv_area_t* clip, lv_draw_rect_dsc_t* dsc)
{
    ...
    if (other_mask_cnt)
        simple_mode = false;
    else if (dsc->border_side != LV_BORDER_SIDE_FULL)
        simple_mode = false;
    ...
    /*Process line by line if there is other mask too*/
        ...
        for (h = draw_area.y1; h <= draw_area.y2; h++) {
            if (normal || (top_only && fill_area.y1 <= coords->y1 + corner_size)
...
LV_ATTRIBUTE_FAST_MEM static void draw_shadow(const lv_area_t* coords, const lv_area_t* clip, lv_draw_rect_dsc_t* dsc)
{
    ...
    if (dsc->shadow_width == 1 && dsc->shadow_ofs_x == 0 && dsc->shadow_ofs_y == 0 && dsc->shadow_spread <= 0) {
        return;
    }
    ...
    if (has_com) {
        ...
        if (w > 0) {
            ...
            for (y = 0; y < h; y++) {
                ...
                _lv_blend_fill(clip, &fa, dsc->shadow_color, mask_buf, mask_res, opa, dsc->shadow_blend_mode);
                fa.y1++;
                fa.y2++;
    ...
    if (has_com) {
        ...
        if (w > 0) {
            ...
            for (y = 0; y < h; y++) {
                ...
                _lv_blend_fill(clip, &fa, dsc->shadow_color, mask_buf, mask_res, dsc->shadow_opa, dsc->shadow_blend_mode);
                fa.y1++;
                fa.y2++;
            }
        }
    ...
    if (has_com) {
        ...
        for (y = 0; y < h; y++) {
            ...
            _lv_blend_fill(clip, &fa, dsc->shadow_color, mask_buf, mask_res, LV_OPA_COVER, dsc->shadow_blend_mode);
            fa.y1++;
            fa.y2++;
            sh_buf_tmp += corner_size;
        }
        ...
        if (mask_res == LV_DRAW_MASK_RES_TRANSP) {
            ...
            for (i = 1; i < size; i++) {
                if (mask_line[i] == mask_line[i - 1])
                    ...
                    sh_ups_tmp_buf[i] = (mask_line[i] << SHADOW_UPSACALE_SHIFT) / sw;
            }
        }
    ...
    if (sw > 1) {
        ...
        for (i = 1; i < (uint32_t)size * size; i++) {
            if (sh_buf[i] == sh_buf[i - 1])
                ...
                sh_buf[i] = (sh_buf[i] << SHADOW_UPSACALE_SHIFT) / sw;
        }
...
static void draw_value(const lv_area_t* coords, const lv_area_t* clip, lv_draw_rect_dsc_t* dsc)
{
    ...
    lv_point_t s;
    _lv_txt_get_size(&s, dsc->value_str, dsc->value_font, dsc->value_letter_space, dsc->value_line_space, LV_COORD_MAX, LV_TXT_FLAG_NONE);
