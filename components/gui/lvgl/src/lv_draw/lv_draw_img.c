 ...
 *  STATIC PROTOTYPES
 **********************/
LV_ATTRIBUTE_FAST_MEM static lv_res_t lv_img_draw_core(const lv_area_t* coords, const lv_area_t* clip_area, const void* src,
                                                       ...
                                                       lv_draw_img_dsc_t* draw_dsc)
{
    ...
    /* The whole uncompressed image is not available. Try to read it line-by-line*/
        ...
        uint8_t* buf = _lv_mem_buf_get(lv_area_get_width(&mask_com) * LV_IMG_PX_SIZE_ALPHA_BYTE); /*+1 because of the possible alpha byte*/
                                              ...
                                              lv_draw_img_dsc_t* draw_dsc, bool chroma_key, bool alpha_byte)
{
    ...
    if (other_mask_cnt == 0 && draw_dsc->angle == 0 && draw_dsc->zoom == LV_IMG_ZOOM_NONE && chroma_key == false && alpha_byte == false
        && draw_dsc->recolor_opa == LV_OPA_TRANSP) {
        _lv_blend_map(clip_area, map_area, (lv_color_t*)map_p, NULL, LV_DRAW_MASK_RES_FULL_COVER, draw_dsc->opa, draw_dsc->blend_mode);
    }
    /*In the other cases every pixel need to be checked one-by-one*/
        ...
        /*Most complicated case: transform or other mask or chroma keyed*/
            ...
            mask_res = (alpha_byte || chroma_key || draw_dsc->angle || draw_dsc->zoom != LV_IMG_ZOOM_NONE) ? LV_DRAW_MASK_RES_CHANGED
            ...
            for (y = 0; y < draw_area_h; y++) {
                ...
                if (px_i + lv_area_get_width(&draw_area) < mask_buf_size) {
                    ...
                    px_i = 0;
                    mask_res = (alpha_byte || chroma_key || draw_dsc->angle || draw_dsc->zoom != LV_IMG_ZOOM_NONE)
