 ...
 */
LV_ATTRIBUTE_FAST_MEM void lv_draw_line(const lv_point_t* point1, const lv_point_t* point2, const lv_area_t* clip, lv_draw_line_dsc_t* dsc)
{
    if (dsc->width == 0)
        return;
                                                ...
                                                lv_draw_line_dsc_t* dsc)
{
    ...
    if (simple_mode) {
        _lv_blend_fill(clip, &draw_area, dsc->color, NULL, LV_DRAW_MASK_RES_FULL_COVER, opa, dsc->blend_mode);
    }
    /*If there other mask apply it*/
        ...
        for (h = draw_area.y1; h <= draw_area.y2; h++) {
            ...
            _lv_blend_fill(clip, &fill_area, dsc->color, mask_buf, mask_res, dsc->opa, dsc->blend_mode);
                                                ...
                                                lv_draw_line_dsc_t* dsc)
{
    ...
    /*If there other mask apply it*/
        ...
        for (h = draw_area.y1; h <= draw_area.y2; h++) {
            ...
            _lv_blend_fill(clip, &fill_area, dsc->color, mask_buf, mask_res, dsc->opa, LV_BLEND_MODE_NORMAL);
                                                 ...
                                                 lv_draw_line_dsc_t* dsc)
{
    ...
    for (h = draw_area.y1 + disp_area->y1; h <= draw_area.y2 + disp_area->y1; h++) {
        ...
        if ((uint32_t)mask_p + draw_area_w < mask_buf_size) {
            ...
            _lv_blend_fill(&fill_area, clip, dsc->color, mask_buf, LV_DRAW_MASK_RES_CHANGED, dsc->opa, dsc->blend_mode);
