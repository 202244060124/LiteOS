...
lv_obj_t* lv_spinbox_create(lv_obj_t* par, const lv_obj_t* copy)
{
    ...
    if (ancestor_signal == NULL)
        ancestor_signal = lv_obj_get_signal_cb(spinbox);
    if (ancestor_design == NULL)
        ancestor_design = lv_obj_get_design_cb(spinbox);
...
void lv_spinbox_set_digit_format(lv_obj_t* spinbox, uint8_t digit_count, uint8_t separator_position)
{
    ...
    if (digit_count < LV_SPINBOX_MAX_DIGIT_COUNT) {
        ...
        if (ext->range_max > max_val - 1)
            ext->range_max = max_val - 1;
        if (ext->range_min < -max_val + 1)
            ext->range_min = -max_val + 1;
    }
...
static lv_res_t lv_spinbox_signal(lv_obj_t* spinbox, lv_signal_t sign, void* param)
{
    ...
    if (sign == LV_SIGNAL_GET_STYLE) {
        ...
        if (info->result != NULL)
            ...
            return ancestor_signal(spinbox, sign, param);
    }
    ...
    if (sign == LV_SIGNAL_CLEANUP) {
        ...
        if (lv_indev_get_type(indev) == LV_INDEV_TYPE_ENCODER) {
            ...
            if (ext->ta.cursor.pos > ext->dec_point_pos && ext->dec_point_pos != 0)
                cp--;
            uint32_t pos = len - cp;
