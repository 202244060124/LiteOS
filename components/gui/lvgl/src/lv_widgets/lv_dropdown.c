...
lv_obj_t* lv_dropdown_create(lv_obj_t* par, const lv_obj_t* copy)
{
    ...
    if (ancestor_signal == NULL)
        ancestor_signal = lv_obj_get_signal_cb(ddlist);
    if (ancestor_design == NULL)
        ancestor_design = lv_obj_get_design_cb(ddlist);
...
void lv_dropdown_open(lv_obj_t* ddlist)
{
    ...
    if (ancestor_page_signal == NULL)
        ancestor_page_signal = lv_obj_get_signal_cb(ext->page);
    if (ancestor_page_scrl_signal == NULL)
        ancestor_page_scrl_signal = lv_obj_get_signal_cb(lv_page_get_scrllable(ext->page));
    if (ancestor_page_design == NULL)
        ancestor_page_design = lv_obj_get_design_cb(ext->page);
    ...
    if (lv_obj_get_width(ext->page) < lv_obj_get_width(ddlist) && (ext->dir == LV_DROPDOWN_DIR_UP || ext->dir == LV_DROPDOWN_DIR_DOWN)) {
    ...
    if (list_h > ext->max_height)
        list_h = ext->max_height;
    ...
    if (dir == LV_DROPDOWN_DIR_DOWN)
        lv_obj_align(ext->page, ddlist, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 0);
    else if (dir == LV_DROPDOWN_DIR_UP)
        lv_obj_align(ext->page, ddlist, LV_ALIGN_OUT_TOP_LEFT, 0, 0);
    else if (dir == LV_DROPDOWN_DIR_LEFT)
        lv_obj_align(ext->page, ddlist, LV_ALIGN_OUT_LEFT_TOP, 0, 0);
    else if (dir == LV_DROPDOWN_DIR_RIGHT)
        lv_obj_align(ext->page, ddlist, LV_ALIGN_OUT_RIGHT_TOP, 0, 0);
...
static lv_design_res_t lv_dropdown_design(lv_obj_t* ddlist, const lv_area_t* clip_area, lv_design_mode_t mode)
{
    ...
    /*Draw the object*/
        ...
        if (txt) {
            _lv_txt_get_size(&txt_size, txt, label_dsc.font, label_dsc.letter_space, label_dsc.line_space, LV_COORD_MAX, label_dsc.flag);
...
static lv_res_t lv_dropdown_signal(lv_obj_t* ddlist, lv_signal_t sign, void* param)
{
    ...
    if (sign == LV_SIGNAL_GET_STYLE) {
        ...
        if (info->part == LV_DROPDOWN_PART_LIST || info->part == LV_DROPDOWN_PART_SCROLLBAR || info->part == LV_DROPDOWN_PART_SELECTED) {
