...
static void draw_header(lv_obj_t* calendar, const lv_area_t* mask)
{
    ...
    header_area.x1 =
        header_area.x2 - header_right - _lv_txt_get_width(LV_SYMBOL_RIGHT, (uint16_t)strlen(LV_SYMBOL_RIGHT), font, 0, LV_TXT_FLAG_NONE);
...
static void draw_dates(lv_obj_t* calendar, const lv_area_t* clip_area)
{
    ...
    for (week = 0; week < 6; week++) {
        ...
        for (day = 0; day < 7; day++) {
            ...
            else if (draw_state == DAY_DRAW_ACT_MONTH && day_cnt > get_month_length(ext->showed_date.year, ext->showed_date.month)) {
...
static bool is_highlighted(lv_obj_t* calendar, day_draw_state_t draw_state, int32_t year, int32_t month, int32_t day)
{
    ...
    for (i = 0; i < ext->highlighted_dates_num; i++) {
        if (ext->highlighted_dates[i].year == year && ext->highlighted_dates[i].month == month && ext->highlighted_dates[i].day == day) {
            return true;
        }
    }
