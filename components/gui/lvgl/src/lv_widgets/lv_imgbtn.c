...
lv_obj_t* lv_imgbtn_create(lv_obj_t* par, const lv_obj_t* copy)
{
    ...
    if (ancestor_signal == NULL)
        ancestor_signal = lv_obj_get_signal_cb(imgbtn);
    if (ancestor_design == NULL)
        ancestor_design = lv_obj_get_design_cb(imgbtn);
...
void lv_imgbtn_set_src_tiled(lv_obj_t* imgbtn, lv_btn_state_t state, const void* src_left, const void* src_mid, const void* src_right)
{
    ...
    if (lv_img_src_get_type(src_left) == LV_IMG_SRC_SYMBOL || lv_img_src_get_type(src_mid) == LV_IMG_SRC_SYMBOL
...
static void refr_img(lv_obj_t* imgbtn)
{
    ...
    if (info_res == LV_RES_OK) {
        ...
        if (ext->tiled)
            lv_obj_set_height(imgbtn, header.h); /*Keep the suer defined width*/
        else
