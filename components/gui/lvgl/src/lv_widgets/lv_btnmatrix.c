        ...
        if (btn_cnt != 0) {
            ...
            for (i = 0; i < btn_cnt; i++) {
                ...
                if (base_dir == LV_BIDI_DIR_RTL) {
                    ...
                    act_x = (unit_act_cnt * all_unit_w) / unit_cnt + i * inner + left;
                }
                /* Set the button's area.
                 * If inner padding is zero then use the prev. button x2 as x1 to avoid rounding
                 * errors*/
                if (btn_i != 0 && inner == 0
                    && ((act_x != left && base_dir != LV_BIDI_DIR_RTL)
    ...
    if (sign == LV_SIGNAL_CLEANUP) {
        ...
        if (indev_type == LV_INDEV_TYPE_POINTER || indev_type == LV_INDEV_TYPE_BUTTON) {
            ...
            if (btn_pr != LV_BTNMATRIX_BTN_NONE) {
                if (button_is_inactive(ext->ctrl_bits[btn_pr]) == false && button_is_hidden(ext->ctrl_bits[btn_pr]) == false) {
                 ...
                 || (indev_type == LV_INDEV_TYPE_ENCODER && lv_group_get_editing(lv_obj_get_group(btnm)))) {
        ...
        if (ext->btn_id_act != LV_BTNMATRIX_BTN_NONE) {
            if (button_is_click_trig(ext->ctrl_bits[ext->btn_id_act]) == false && button_is_inactive(ext->ctrl_bits[ext->btn_id_act]) == false
        ...
        if (btn_pr != ext->btn_id_pr) {
            ...
            if (btn_pr != LV_BTNMATRIX_BTN_NONE && button_is_inactive(ext->ctrl_bits[btn_pr]) == false
        ...
        if (ext->btn_id_pr != LV_BTNMATRIX_BTN_NONE) {
            ...
            if (button_is_tgl_enabled(ext->ctrl_bits[ext->btn_id_pr]) && !button_is_inactive(ext->ctrl_bits[ext->btn_id_pr])) {
            ...
            if (button_is_click_trig(ext->ctrl_bits[ext->btn_id_act]) == true && button_is_inactive(ext->ctrl_bits[ext->btn_id_act]) == false
                && button_is_hidden(ext->ctrl_bits[ext->btn_id_act]) == false) {
                uint32_t b = ext->btn_id_act;
                res = lv_event_send(btnm, LV_EVENT_VALUE_CHANGED, &b);
            }
            ...
            if (ext->btn_id_focused == LV_BTNMATRIX_BTN_NONE) {
                ...
                for (area_below = ext->btn_id_focused; area_below < ext->btn_cnt; area_below++) {
                    if (ext->button_areas[area_below].y1 > ext->button_areas[ext->btn_id_focused].y1
                        && pr_center >= ext->button_areas[area_below].x1 && pr_center <= ext->button_areas[area_below].x2 + pad_inner
                        && button_is_inactive(ext->ctrl_bits[area_below]) == false && button_is_hidden(ext->ctrl_bits[area_below]) == false) {
                        break;
                    }
                }
            ...
            if (ext->btn_id_focused == LV_BTNMATRIX_BTN_NONE) {
                ...
                for (area_above = ext->btn_id_focused; area_above >= 0; area_above--) {
                    if (ext->button_areas[area_above].y1 < ext->button_areas[ext->btn_id_focused].y1
                        && pr_center >= ext->button_areas[area_above].x1 - pad_inner && pr_center <= ext->button_areas[area_above].x2
                        && button_is_inactive(ext->ctrl_bits[area_above]) == false && button_is_hidden(ext->ctrl_bits[area_above]) == false) {
                        break;
                    }
                }
                if (area_above >= 0)
...
static void allocate_btn_areas_and_controls(const lv_obj_t* btnm, const char** map)
{
    ...
    if (ext->button_areas == NULL || ext->ctrl_bits == NULL)
        btn_cnt = 0;
...
static uint16_t get_button_from_point(lv_obj_t* btnm, lv_point_t* p)
{
    ...
    for (i = 0; i < ext->btn_cnt; i++) {
        ...
        if (btn_area.x2 >= w - pright - 2)
            btn_area.x2 = btnm_cords.x2; /*-2 for rounding error*/
        else
            btn_area.x2 += btnm_cords.x1 + pinner;

        if (btn_area.y2 >= h - pbottom - 2)
            btn_area.y2 = btnm_cords.y2; /*-2 for rounding error*/
        else
