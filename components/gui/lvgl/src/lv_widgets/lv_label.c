...
static lv_design_res_t lv_label_design(lv_obj_t* label, const lv_area_t* clip_area, lv_design_mode_t mode)
{
    ...
    if (mode == LV_DESIGN_COVER_CHK)
        ...
        if ((ext->long_mode == LV_LABEL_LONG_SROLL || ext->long_mode == LV_LABEL_LONG_SROLL_CIRC)
            ...
            lv_point_t size;
            _lv_txt_get_size(&size, ext->text, label_draw_dsc.font, label_draw_dsc.letter_space, label_draw_dsc.line_space, LV_COORD_MAX,
                             flag);
        ...
        if (ext->long_mode == LV_LABEL_LONG_SROLL_CIRC) {
            ...
            if (size.x > lv_area_get_width(&txt_coords)) {
                label_draw_dsc.ofs_x =
                    ext->offset.x + size.x + lv_font_get_glyph_width(label_draw_dsc.font, ' ', ' ') * LV_LABEL_WAIT_CHAR_COUNT;
                label_draw_dsc.ofs_y = ext->offset.y;
...
static void lv_label_refr_text(lv_obj_t* label)
{
    ...
    /*In roll mode keep the size but start offset animations*/
        ...
        lv_anim_set_playback_delay(&a, (((lv_font_get_glyph_width(font, ' ', ' ') + letter_space) * 1000) / ext->anim_speed)
    ...
    /*In roll inf. mode keep the size but start offset animations*/
        ...
        if (size.y <= lv_area_get_height(&txt_coords)) {                        /*No dots are required, the text is short enough*/
            ...
            p.x = lv_area_get_width(&txt_coords)
                  - (lv_font_get_glyph_width(font, '.', '.') + letter_space) * LV_LABEL_DOT_NUM; /*Shrink with dots*/
            p.y = lv_area_get_height(&txt_coords);
            p.y -= p.y % (lv_font_get_line_height(font) + line_space);                           /*Round down to the last line*/
