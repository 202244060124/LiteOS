...
lv_obj_t* lv_checkbox_create(lv_obj_t* par, const lv_obj_t* copy)
{
    ...
    if (ancestor_signal == NULL)
        ancestor_signal = lv_obj_get_signal_cb(cb);
...
static lv_res_t lv_checkbox_signal(lv_obj_t* cb, lv_signal_t sign, void* param)
{
    ...
    if (sign == LV_SIGNAL_GET_STYLE) {
        ...
        if (info->result != NULL)
            ...
            return ancestor_signal(cb, sign, param);
    }
    ...
    } else if (sign == LV_SIGNAL_PRESSED || sign == LV_SIGNAL_RELEASED || sign == LV_SIGNAL_PRESS_LOST || sign == LV_SIGNAL_FOCUS
