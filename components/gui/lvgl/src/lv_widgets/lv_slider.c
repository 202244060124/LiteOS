...
static lv_res_t lv_slider_signal(lv_obj_t* slider, lv_signal_t sign, void* param)
{
    ...
    if (sign == LV_SIGNAL_PRESSED) {
        ...
        if (lv_slider_get_type(slider) == LV_SLIDER_TYPE_NORMAL) {
            ...
            if (hor) {
                if ((base_dir != LV_BIDI_DIR_RTL && p.x > ext->right_knob_area.x2)
                    || (base_dir == LV_BIDI_DIR_RTL && p.x < ext->right_knob_area.x1)) {
                    ext->value_to_set = &ext->bar.cur_value;
                } else if ((base_dir != LV_BIDI_DIR_RTL && p.x < ext->left_knob_area.x1)
                           || (base_dir == LV_BIDI_DIR_RTL && p.x > ext->left_knob_area.x2)) {
        ...
        if (lv_obj_get_width(slider) != lv_area_get_width(param) || lv_obj_get_height(slider) != lv_area_get_height(param)) {
