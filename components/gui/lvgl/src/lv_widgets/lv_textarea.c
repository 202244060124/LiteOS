...
static lv_design_res_t lv_textarea_scrollable_design(lv_obj_t* scrl, const lv_area_t* clip_area, lv_design_mode_t mode);
...
lv_obj_t* lv_textarea_create(lv_obj_t* par, const lv_obj_t* copy)
{
    ...
    if (scrl_signal == NULL)
        scrl_signal = lv_obj_get_signal_cb(lv_page_get_scrllable(ta));
    if (scrl_design == NULL)
        scrl_design = lv_obj_get_design_cb(lv_page_get_scrllable(ta));
...
void lv_textarea_set_cursor_pos(lv_obj_t* ta, int16_t pos)
{
    ...
    if (pos > len || pos == LV_TEXTAREA_CURSOR_LAST)
        pos = len;
...
static lv_res_t lv_textarea_signal(lv_obj_t* ta, lv_signal_t sign, void* param)
{
    ...
    if (sign == LV_SIGNAL_CLEANUP) {
        ...
        *editable = true;
    } else if (sign == LV_SIGNAL_PRESSED || sign == LV_SIGNAL_PRESSING || sign == LV_SIGNAL_PRESS_LOST || sign == LV_SIGNAL_RELEASED) {
...
static lv_res_t lv_textarea_scrollable_signal(lv_obj_t* scrl, lv_signal_t sign, void* param)
{
    ...
    } else if (sign == LV_SIGNAL_PRESSING || sign == LV_SIGNAL_PRESSED || sign == LV_SIGNAL_PRESS_LOST || sign == LV_SIGNAL_RELEASED) {
        update_cursor_position_on_click(ta, sign, (lv_indev_t*)param);
    }
...
static bool char_is_accepted(lv_obj_t* ta, uint32_t c)
{
    ...
    if (ext->accapted_chars == NULL && ext->max_length == 0)
        return true;
...
static void update_cursor_position_on_click(lv_obj_t* ta, lv_signal_t sign, lv_indev_t* click_source)
{
    ...
    if (lv_indev_get_type(click_source) == LV_INDEV_TYPE_KEYPAD || lv_indev_get_type(click_source) == LV_INDEV_TYPE_ENCODER) {
        return;
    }
