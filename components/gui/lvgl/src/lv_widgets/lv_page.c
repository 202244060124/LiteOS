...
static lv_res_t lv_page_signal(lv_obj_t* page, lv_signal_t sign, void* param)
{
    ...
    if (sign == LV_SIGNAL_GET_STYLE) {
        ...
        if (info->part == LV_PAGE_PART_SCROLLABLE)
            info->result = lv_obj_get_state(lv_page_get_scrllable(page), LV_CONT_PART_MAIN);
        else
    ...
    /*Automatically move children to the scrollable object*/
        ...
        while (child != NULL) {
            if (lv_obj_is_protected(child, LV_PROTECT_PARENT) == false) {
                ...
                if ((tmp->coords.x1 == page->coords.x1) && (fit_left == LV_FIT_TIGHT || fit_left == LV_FIT_MAX)
                    && base_dir != LV_BIDI_DIR_RTL) {
                    tmp->coords.x1 += scrl_left;
                    tmp->coords.x2 += scrl_left;
                } else if ((tmp->coords.x2 == page->coords.x2) && (fit_right == LV_FIT_TIGHT || fit_right == LV_FIT_MAX)
                           && base_dir == LV_BIDI_DIR_RTL) {
        ...
        if (ext->scrl != NULL
            && (lv_obj_get_width(page) != lv_area_get_width(param) || lv_obj_get_height(page) != lv_area_get_height(param))) {
...
static void scrl_def_event_cb(lv_obj_t* scrl, lv_event_t event)
{
    ...
    if (event == LV_EVENT_PRESSED || event == LV_EVENT_PRESSING || event == LV_EVENT_PRESS_LOST || event == LV_EVENT_RELEASED
        ...
        || event == LV_EVENT_LONG_PRESSED_REPEAT || event == LV_EVENT_DRAG_BEGIN || event == LV_EVENT_DRAG_END
        || event == LV_EVENT_DRAG_THROW_BEGIN) {
...
static void scrlbar_refresh(lv_obj_t* page)
{
    ...
    if (scrl_w <= obj_w - bg_left - bg_right) {
        ...
        _lv_area_set_pos(&ext->scrlbar.hor_area, sb_hor_pad, obj_h - sb_width - sb_bottom);
        if (ext->scrlbar.mode == LV_SCROLLBAR_MODE_AUTO || ext->scrlbar.mode == LV_SCROLLBAR_MODE_DRAG)
            ext->scrlbar.hor_draw = 0;
    }
    /*Smaller horizontal scrollbar*/
    else {
        size_tmp = (obj_w * (obj_w - (2 * sb_hor_pad))) / (scrl_w + bg_left + bg_right);
    ...
    if (scrl_h <= obj_h - bg_top - bg_bottom) {
        ...
        _lv_area_set_pos(&ext->scrlbar.ver_area, obj_w - sb_width - sb_right, sb_ver_pad);
        if (ext->scrlbar.mode == LV_SCROLLBAR_MODE_AUTO || ext->scrlbar.mode == LV_SCROLLBAR_MODE_DRAG)
            ext->scrlbar.ver_draw = 0;
    }
    /*Smaller vertical scroll bar*/
        ...
        _lv_area_set_pos(&ext->scrlbar.ver_area, obj_w - sb_width - sb_right,
                         sb_ver_pad
                             + (-(lv_obj_get_y(scrl) - bg_left) * (obj_h - size_tmp - 2 * sb_ver_pad))
