...
lv_obj_t* lv_list_create(lv_obj_t* par, const lv_obj_t* copy)
{
    ...
    if (ancestor_page_signal == NULL)
        ancestor_page_signal = lv_obj_get_signal_cb(list);
    ...
    if (copy == NULL) {
        ...
        while (copy_btn) {
            ...
            if (copy_img)
                img_src = lv_img_get_src(copy_img);
#endif
...
lv_obj_t* lv_list_add_btn(lv_obj_t* list, const void* img_src, const char* txt)
{
    ...
    if (ancestor_btn_signal == NULL)
        ancestor_btn_signal = lv_obj_get_signal_cb(btn);
    ...
    if (img_src) {
        ...
        if (img_signal == NULL)
            img_signal = lv_obj_get_signal_cb(img);
    }
#endif
    if (txt != NULL) {
        ...
        if (label_signal == NULL)
            label_signal = lv_obj_get_signal_cb(label);
    }
...
static lv_res_t lv_list_signal(lv_obj_t* list, lv_signal_t sign, void* param)
{
    ...
    if (sign == LV_SIGNAL_GET_STYLE) {
        ...
        if (info->result != NULL)
            ...
            return ancestor_page_signal(list, sign, param);
    }
    ...
    if (sign == LV_SIGNAL_RELEASED || sign == LV_SIGNAL_PRESSED || sign == LV_SIGNAL_PRESSING || sign == LV_SIGNAL_LONG_PRESS
        ...
        if (indev_type == LV_INDEV_TYPE_KEYPAD || (indev_type == LV_INDEV_TYPE_ENCODER && lv_group_get_editing(lv_obj_get_group(list)))) {
            lv_list_ext_t* ext = lv_obj_get_ext_attr(list);
        ...
        lv_group_t* g = lv_obj_get_group(list);
        if ((indev_type == LV_INDEV_TYPE_KEYPAD) || (indev_type == LV_INDEV_TYPE_ENCODER && lv_group_get_editing(g))) {
            lv_list_ext_t* ext = lv_obj_get_ext_attr(list);
            /* Select the last used button, or use the first no last button */
            if (ext->last_sel_btn)
                lv_list_focus_btn(list, ext->last_sel_btn);
            else
...
static lv_res_t lv_list_btn_signal(lv_obj_t* btn, lv_signal_t sign, void* param)
{
    ...
    if ((sign == LV_SIGNAL_COORD_CHG
         && (lv_obj_get_width(btn) != lv_area_get_width(param) || lv_obj_get_height(btn) != lv_area_get_height(param)))
