...
lv_obj_t* lv_tabview_create(lv_obj_t* par, const lv_obj_t* copy)
{
    ...
    if (ancestor_signal == NULL)
        ancestor_signal = lv_obj_get_signal_cb(tabview);
    ...
    if (copy == NULL) {
        ...
        if (ancestor_scrl_signal == NULL)
            ancestor_scrl_signal = lv_obj_get_signal_cb(lv_page_get_scrllable(ext->content));
    ...
    /*Copy an existing tab view*/
        ...
        lv_style_list_copy(lv_obj_get_style_list(tabview, LV_TABVIEW_PART_BG_SCRLLABLE),
                           lv_obj_get_style_list(copy, LV_TABVIEW_PART_BG_SCRLLABLE));
        ...
        lv_style_list_copy(lv_obj_get_style_list(tabview, LV_TABVIEW_PART_TAB_BTN), lv_obj_get_style_list(copy, LV_TABVIEW_PART_TAB_BTN));
        ...
        for (i = 0; i < copy_ext->tab_cnt; i++) {
            ...
            lv_style_list_copy(lv_obj_get_style_list(new_tab, LV_PAGE_PART_SCROLLABLE),
                               lv_obj_get_style_list(copy_tab, LV_PAGE_PART_SCROLLABLE));
            lv_style_list_copy(lv_obj_get_style_list(new_tab, LV_PAGE_PART_SCROLLBAR),
                               lv_obj_get_style_list(copy_tab, LV_PAGE_PART_SCROLLBAR));
...
lv_obj_t* lv_tabview_add_tab(lv_obj_t* tabview, const char* name)
{
    ...
    if (page_signal == NULL)
        page_signal = lv_obj_get_signal_cb(h);
...
{
    if (btns_pos != LV_TABVIEW_TAB_POS_NONE && btns_pos != LV_TABVIEW_TAB_POS_TOP && btns_pos != LV_TABVIEW_TAB_POS_BOTTOM
        && btns_pos != LV_TABVIEW_TAB_POS_LEFT && btns_pos != LV_TABVIEW_TAB_POS_RIGHT) {
...
lv_obj_t* lv_tabview_get_tab(const lv_obj_t* tabview, uint16_t id)
{
    ...
    while (page != NULL && i != id) {
        if (lv_obj_get_signal_cb(page) == page_signal)
            i++;
...
static lv_res_t lv_tabview_signal(lv_obj_t* tabview, lv_signal_t sign, void* param)
{
    ...
    if (sign == LV_SIGNAL_GET_STYLE) {
        ...
        if (info->result != NULL)
            ...
            return ancestor_signal(tabview, sign, param);
    } else if (sign == LV_SIGNAL_GET_STATE_DSC) {
        lv_tabview_ext_t* ext = lv_obj_get_ext_attr(tabview);
        lv_get_state_info_t* info = param;
        if (info->part == LV_TABVIEW_PART_TAB_BG)
            info->result = lv_obj_get_state(ext->btns, LV_BTNMATRIX_PART_BG);
        else if (info->part == LV_TABVIEW_PART_TAB_BTN)
            info->result = lv_obj_get_state(ext->btns, LV_BTNMATRIX_PART_BTN);
        else if (info->part == LV_TABVIEW_PART_INDIC)
            info->result = lv_obj_get_state(ext->indic, LV_OBJ_PART_MAIN);
        else if (info->part == LV_TABVIEW_PART_BG_SCRLLABLE)
            info->result = lv_obj_get_state(ext->content, LV_PAGE_PART_SCROLLABLE);
        return LV_RES_OK;
    }
    ...
    if (sign == LV_SIGNAL_CLEANUP) {
        ...
        if (ext->content != NULL
            && (lv_obj_get_width(tabview) != lv_area_get_width(param) || lv_obj_get_height(tabview) != lv_area_get_height(param))) {
            tabview_realign(tabview);
        }
        ...
        if (indev_type == LV_INDEV_TYPE_KEYPAD || (indev_type == LV_INDEV_TYPE_ENCODER && lv_group_get_editing(lv_obj_get_group(tabview)))) {
...
static lv_res_t tabview_scrl_signal(lv_obj_t* tabview_scrl, lv_signal_t sign, void* param)
{
    ...
    if (sign == LV_SIGNAL_DRAG_THROW_BEGIN) {
        ...
        if (id_prev != id_new)
            res = lv_event_send(tabview, LV_EVENT_VALUE_CHANGED, &id_prev);
        if (res != LV_RES_OK)
            return res;
    }
    return res;
...
static void tab_btnm_event_cb(lv_obj_t* tab_btnm, lv_event_t event)
{
    ...
    if (lv_btnmatrix_get_btn_ctrl(tab_btnm, btn_id, LV_BTNMATRIX_CTRL_DISABLED))
        return;
...
static void refr_btns_size(lv_obj_t* tabview)
{
    ...
    switch (ext->btns_pos) {
        ...
        case LV_TABVIEW_TAB_POS_RIGHT:
            lv_obj_set_hidden(ext->btns, false);
            btns_w = lv_font_get_glyph_width(font, 'A', '\0') + tab_left + tab_right + tab_bg_left + tab_bg_right;
