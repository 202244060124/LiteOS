...
void lv_chart_set_range(lv_obj_t* chart, lv_coord_t ymin, lv_coord_t ymax)
{
    ...
    lv_chart_ext_t* ext = lv_obj_get_ext_attr(chart);
    if (ext->ymin == ymin && ext->ymax == ymax)
        return;
...
static void draw_series_line(lv_obj_t* chart, const lv_area_t* series_area, const lv_area_t* clip_area)
{
    ...
    if (has_area) {
        ...
        if (has_fade) {
            lv_draw_mask_fade_init(&mask_fade_p, series_area, area_dsc.bg_main_color_stop, series_area->y1, area_dsc.bg_grad_color_stop,
                                   series_area->y2);
        }
    }
...
{
    lv_chart_ext_t* ext = lv_obj_get_ext_attr(chart);
    lv_chart_axis_cfg_t* y_axis = (which_axis == LV_CHART_AXIS_PRIMARY_Y) ? &ext->y_axis : &ext->secondary_y_axis;
    ...
    lv_style_int_t label_dist = which_axis == LV_CHART_AXIS_PRIMARY_Y ? lv_obj_get_style_pad_left(chart, LV_CHART_PART_SERIES_BG)
    ...
    for (i = 0; i < (num_scale_ticks + 1); i++) { /* one extra loop - it may not exist in the list, empty label */
        ...
        p2.y = p1.y = y_ofs + (int32_t)((int32_t)(h - line_dsc.width) * i) / num_scale_ticks;
...
static void draw_x_ticks(lv_obj_t* chart, const lv_area_t* series_area, const lv_area_t* mask)
{
    ...
    if (h + y_ofs > mask->y2)
        return;
    if (h + y_ofs + label_dsc.font->line_height + label_dist < mask->y1)
        return;
    ...
    for (i = 0; i < (num_scale_ticks + 1); i++) { /* one extra loop - it may not exist in the list, empty label */
        ...
        if (num_of_labels != 0) {
            ...
            if (is_tick_with_label(i, &(ext->x_axis))) {
                ...
                lv_area_t a = {(p2.x - size.x / 2), (p2.y + label_dist), (p2.x + size.x / 2), (p2.y + size.y + label_dist)};
                lv_draw_label(&a, mask, &label_dsc, buf, NULL);
            }
        }
    }
