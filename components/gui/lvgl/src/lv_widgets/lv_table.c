...
static lv_coord_t get_row_height(lv_obj_t* table, uint16_t row_id, const lv_font_t** font, lv_style_int_t* letter_space,
                                 lv_style_int_t* line_space, lv_style_int_t* cell_left, lv_style_int_t* cell_right,
                                 lv_style_int_t* cell_top, lv_style_int_t* cell_bottom);
...
lv_obj_t* lv_table_create(lv_obj_t* par, const lv_obj_t* copy)
{
    ...
    if (ancestor_signal == NULL)
        ancestor_signal = lv_obj_get_signal_cb(table);
    if (ancestor_design == NULL)
        ancestor_design = lv_obj_get_design_cb(table);
...
void lv_table_set_cell_type(lv_obj_t* table, uint16_t row, uint16_t col, uint8_t type)
{
    ...
    if (type > 0)
        type--; /*User gives 1,2,3,4 but easier to handle 0, 1, 2, 3*/
    if (type >= LV_TABLE_CELL_STYLE_CNT)
        type = LV_TABLE_CELL_STYLE_CNT - 1;
...
static lv_design_res_t lv_table_design(lv_obj_t* table, const lv_area_t* clip_area, lv_design_mode_t mode)
{
    ...
    /*Draw the object*/
        ...
        uint16_t i;
        for (i = 0; i < LV_TABLE_CELL_STYLE_CNT; i++) {
            if ((ext->cell_types & (1 << i)) == 0)
                continue; /*Skip unused cell types*/
        ...
        for (row = 0; row < ext->row_cnt; row++) {
            ...
            for (col = 0; col < ext->col_cnt; col++) {
                ...
                if (ext->cell_data[cell]) {
                    ...
                    _lv_txt_get_size(&txt_size, ext->cell_data[cell] + 1, label_dsc[cell_type].font, label_dsc[cell_type].letter_space,
                                     label_dsc[cell_type].line_space, lv_area_get_width(&txt_area), txt_flags);
...
static lv_res_t lv_table_signal(lv_obj_t* table, lv_signal_t sign, void* param)
{
    ...
    if (sign == LV_SIGNAL_GET_STYLE) {
        ...
        if (info->result != NULL)
            ...
            return ancestor_signal(table, sign, param);
    }
...
static void refr_size(lv_obj_t* table)
{
    ...
    for (i = 0; i < ext->row_cnt; i++) {
        ext->row_h[i] = get_row_height(table, i, font, letter_space, line_space, cell_left, cell_right, cell_top, cell_bottom);
        h += ext->row_h[i];
    }
    ...
    for (cell = row_start, col = 0; cell < row_start + ext->col_cnt; cell++, col++) {
        if (ext->cell_data[cell] != NULL) {
            ...
            /*Without text crop calculate the height of the text in the cell*/
                ...
                _lv_txt_get_size(&txt_size, ext->cell_data[cell] + 1, font[cell_type], letter_space[cell_type], line_space[cell_type],
                                 txt_w, LV_TXT_FLAG_NONE);
