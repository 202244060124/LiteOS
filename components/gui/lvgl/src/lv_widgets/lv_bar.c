...
lv_obj_t* lv_bar_create(lv_obj_t* par, const lv_obj_t* copy)
{
    ...
    if (ancestor_signal == NULL)
        ancestor_signal = lv_obj_get_signal_cb(bar);
    if (ancestor_design_f == NULL)
        ancestor_design_f = lv_obj_get_design_cb(bar);
...
void lv_bar_set_start_value(lv_obj_t* bar, int16_t start_value, lv_anim_enable_t anim)
{
    ...
    lv_bar_ext_t* ext = lv_obj_get_ext_attr(bar);
    if (ext->start_value == start_value)
        return;
...
static void draw_indic(lv_obj_t* bar, const lv_area_t* clip_area)
{
    ...
    bool sym = false;
    if (ext->type == LV_BAR_TYPE_SYMMETRICAL && ext->min_value < 0 && ext->max_value > 0 && ext->start_value == ext->min_value)
        sym = true;
    ...
    if (LV_BAR_IS_ANIMATING(ext->start_value_anim)) {
        lv_coord_t anim_start_value_start_x = (int32_t)((int32_t)anim_length * (ext->start_value_anim.anim_start - ext->min_value)) / range;
        lv_coord_t anim_start_value_end_x = (int32_t)((int32_t)anim_length * (ext->start_value_anim.anim_end - ext->min_value)) / range;

        anim_start_value_x =
            (((anim_start_value_end_x - anim_start_value_start_x) * ext->start_value_anim.anim_state) / LV_BAR_ANIM_STATE_END);
    ...
    if (LV_BAR_IS_ANIMATING(ext->cur_value_anim)) {
        lv_coord_t anim_cur_value_start_x = (int32_t)((int32_t)anim_length * (ext->cur_value_anim.anim_start - ext->min_value)) / range;
        lv_coord_t anim_cur_value_end_x = (int32_t)((int32_t)anim_length * (ext->cur_value_anim.anim_end - ext->min_value)) / range;

        anim_cur_value_x = anim_cur_value_start_x
                           + (((anim_cur_value_end_x - anim_cur_value_start_x) * ext->cur_value_anim.anim_state) / LV_BAR_ANIM_STATE_END);
    } else
#endif
    {
    ...
    if (!sym && indic_length_calc(&ext->indic_area) <= 1)
        return;
    ...
    if ((hor && lv_area_get_width(&ext->indic_area) > bg_radius * 2) || (!hor && lv_area_get_height(&ext->indic_area) > bg_radius * 2)) {
...
static lv_res_t lv_bar_signal(lv_obj_t* bar, lv_signal_t sign, void* param)
{
    ...
    if (sign == LV_SIGNAL_GET_STYLE) {
        ...
        if (info->result != NULL)
            ...
            return ancestor_signal(bar, sign, param);
    }
