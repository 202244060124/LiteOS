...
lv_obj_t* lv_spinner_create(lv_obj_t* par, const lv_obj_t* copy)
{
    ...
    if (ancestor_signal == NULL)
        ancestor_signal = lv_obj_get_signal_cb(spinner);
    if (ancestor_design == NULL)
        ancestor_design = lv_obj_get_design_cb(spinner);
...
void lv_spinner_set_type(lv_obj_t* spinner, lv_spinner_type_t type)
{
    ...
    switch (type) {
        case LV_SPINNER_TYPE_FILLSPIN_ARC: {
            ...
            if (ext->anim_dir == LV_SPINNER_DIR_FORWARD)
                lv_anim_set_values(&a, ext->arc_length, 360 - ext->arc_length);
            else
                lv_anim_set_values(&a, 360 - ext->arc_length, ext->arc_length);
