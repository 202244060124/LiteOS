...
void messageArrived(MessageData* md)
{
    ...
    if (pubmsg.payloadlen == m->payloadlen)
        assert("Good message contents", memcmp(m->payload, pubmsg.payload, m->payloadlen) == 0, "payload was %s", m->payload);
}
...
int check_subs_exist(MQTTClient* c, const char* test_topic, int which)
{
    ...
    rc = (((which == 1 || which == 3) && test1_message_data) || (which == 2 && test1_message_data == NULL)) ? MQTT_SUCCESS : FAILURE;
    assert("test1 subscription", rc == MQTT_SUCCESS, "test1_message_data %p\n", test1_message_data);
    rc = (((which == 2 || which == 3) && test2_message_data) || (which == 1 && test2_message_data == NULL)) ? MQTT_SUCCESS : FAILURE;
