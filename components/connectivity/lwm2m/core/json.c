...
typedef enum { _STEP_START, _STEP_TOKEN, _STEP_ANY_SEPARATOR, _STEP_SEPARATOR, _STEP_QUOTED_VALUE, _STEP_VALUE, _STEP_DONE } _itemState;
...
static int prv_split(uint8_t* buffer, size_t bufferLen, int* tokenStartP, int* tokenLenP, int* valueStartP, int* valueLenP)
{
    size_t index;
    _itemState step;
            ...
            case 1: {
                switch (buffer[index + tokenStart]) {
                    case 'n': {
                        ...
                        if (valueLen < 3 || buffer[index + valueStart] != '"' || buffer[index + valueStart + valueLen - 1] != '"') {
                            return -1;
                        }
                        // Ignore starting /
            ...
            case 2: {
                ...
                switch (buffer[index + tokenStart]) {
                    ...
                    case 's':
                        ...
                        if (valueLen < 2 || buffer[index + valueStart] != '"' || buffer[index + valueStart + valueLen - 1] != '"') {
                            return -1;
                        }
    ...
    for (i = 0; i < size; i++) {
        if (dataP[i].type != LWM2M_TYPE_UNDEFINED) {
            ...
            if (dataP[i].type == LWM2M_TYPE_OBJECT || dataP[i].type == LWM2M_TYPE_OBJECT_INSTANCE
                ...
                childLen = prv_dataStrip(dataP[i].value.asChildren.count, dataP[i].value.asChildren.array,
...
static int prv_findAndCheckData(lwm2m_uri_t* uriP, uri_depth_t level, size_t size, lwm2m_data_t* tlvP, lwm2m_data_t** targetP)
{
    size_t index;
    int result;
                ...
                case LWM2M_TYPE_OBJECT:
                    for (index = 0; index < size; index++) {
                        if (tlvP[index].id == uriP->objectId) {
                            return prv_findAndCheckData(uriP, level, tlvP[index].value.asChildren.count, tlvP[index].value.asChildren.array,
                                                        targetP);
                        }
                    }
                    break;
...
int json_serialize(lwm2m_uri_t* uriP, int size, lwm2m_data_t* tlvP, uint8_t** bufferP)
{
    ...
    while (num == 1
           && (targetP->type == LWM2M_TYPE_OBJECT || targetP->type == LWM2M_TYPE_OBJECT_INSTANCE
               || targetP->type == LWM2M_TYPE_MULTIPLE_RESOURCE)) {
        int res;
