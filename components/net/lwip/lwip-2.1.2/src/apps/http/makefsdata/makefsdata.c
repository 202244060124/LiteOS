...
/* OUT_BUF_SIZE is the size of the output buffer used during decompression.
   OUT_BUF_SIZE must be a power of 2 >= TINFL_LZ_DICT_SIZE (because the low-level decompressor not only writes, but reads from the output
   buffer as it decompresses) */
...
int file_write_http_header(FILE* data_file, const char* filename, int file_size, u16_t* http_hdr_len, u16_t* http_hdr_chksum,
                           u8_t provide_content_len, int is_compressed);
...
int main(int argc, char* argv[])
{
...
#if ALIGN_PAYLOAD
    fprintf(data_file, "/* FSDATA_FILE_ALIGNMENT: 0=off, 1=by variable, 2=by include */" NEWLINE "#ifndef FSDATA_FILE_ALIGNMENT" NEWLINE
                       "#define FSDATA_FILE_ALIGNMENT 0" NEWLINE "#endif" NEWLINE);
#endif
    ...
    if (deflateNonSsiFiles) {
        printf("(Deflated total byte reduction: %d bytes -> %d bytes (%.02f%%)" NEWLINE, (int)overallDataBytes, (int)deflatedBytesReduced,
            ...
            if (fsize < OUT_BUF_SIZE) {
                ...
                if (out_bytes < fsize) {
                    ...
                    memcpy(ret_buf, s_outbuf, out_bytes);
                    {
                        ...
                        dec_status = tinfl_decompress(&inflator, (const mz_uint8*)ret_buf, &dec_in_bytes, s_checkbuf, (mz_uint8*)next_out,
                                                      &dec_out_bytes, 0);
