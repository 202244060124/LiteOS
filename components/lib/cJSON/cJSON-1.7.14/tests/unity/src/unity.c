...
void UnityPrintFloat(const UNITY_DOUBLE input_number)
{
    ...
    if (number == (double)0.0f)
        ...
        while (number < (double)(100000.0f / 1e6f)) {
            number *= (double)1e6f;
            exponent -= 6;
        }
        while (number < (double)100000.0f) {
            number *= (double)10.0f;
            exponent--;
        }
        while (number > (double)(1000000.0f * 1e6f)) {
            number /= (double)1e6f;
            exponent += 6;
        }
        while (number > (double)1000000.0f) {
            number /= (double)10.0f;
            exponent++;
        }
    ...
    while (elements--) {
        ...
        if (expect_val != actual_val) {
            if (style & UNITY_DISPLAY_RANGE_UINT
                && length < sizeof(expect_val)) { /* For UINT, remove sign extension (padding 1's) from signed type casts above */
                UNITY_INT mask = 1;
